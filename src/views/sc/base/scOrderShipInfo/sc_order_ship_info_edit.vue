<template>
	<el-dialog title="海运船名 - 编辑" width="560px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
		<el-form :disabled="loading" :model="ruleForm" :rules="rules" ref="ruleForm" style="margin-left: 10px;">
			...
			<el-row>
				<el-col class="elementWidth">
					<el-form-item>
							<el-input class="widthWithThreec" clearable auto-complete="off" style="width:190px;margin-right: 0px;">
								<template slot="prepend">
									<span>是否生效</span>
								</template>
...
<script>
	export default {
		...
		methods: {
			...
			init() {
			   this.ruleForm.shipNameEn = this.frow.shipNameEn;
			   this.ruleForm.shipNameCn = this.frow.shipNameCn;
			   this.ruleForm.shipInfoId = this.frow.shipInfoId;
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						this.$axios.post2("/sc/orderShipInfo/doUpdate", params)
