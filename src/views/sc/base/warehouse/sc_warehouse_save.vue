...
<script>
	export default {
     ...
     let param = {};
     this.$axios.get("/sc/portMaintenance/page?size=10&current=1",param).then(function(response) {
     ...
     this.$axios.get('/sc/portMaintenance?').then(function(response) {
       ...
       // this.portMaintenanceList = response.data.data.slice(0, 10)
     	// this.portMaintenanceList = response.data.data;
     }.bind(this));
      	...
      	if (val) {
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post2('/sc/scWarehouse/doSave', this.ruleForm)
