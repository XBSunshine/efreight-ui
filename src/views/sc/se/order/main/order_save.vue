		...
		<div class="orderEditPage" style="width: 1280px;">
			<el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
				<div style="margin-top: -15px;margin-bottom: 20px;">
					<span style="font-size: large;margin-bottom: 20px;">海运出口订单&nbsp;-&nbsp;新增</span>
      ...
      search(url, val, callback){
        ...
        this.search("/sc/portMaintenance/search/", val, function(data){
			...
			onChangeDepartureStation(val) {
				...
				this.$axios.get2('/sc/scWarehouse/list', a).then((response) => {
					this.scWarehouseListTwo = response.data.data;
				})
			},
			...
			changeActiveOperationName(val) {
				if (val == 'TG') {
					...
					if (!this.ruleForm.containerLoadService) {
						...
						this.ruleForm.containerLoadWarehouseId = '';
						this.ruleForm.containerLoadAddressCn = '';
						this.scWarehouseList = [];
					} else {
						this.scWarehouseList = this.scWarehouseListTwo;
					}
		...
		created() {
			...
			this.$axios.get('/sc/vScCategory/运输条款').then((response) => {
				...
				if (this.transitClauses.length != 0) {
					this.ruleForm.transitClause = this.transitClauses[0].paramText
				}
			})
			...
			this.$axios.get('/sc/vScCategory/运费条款').then((response) => {
				...
				if (this.paymentMethods.length != 0) {
					this.ruleForm.paymentMethod = this.paymentMethods[0].paramText
				}
			})
			...
			//查询签约公司默认配置信息
			this.$axios.get('/hrs/org/getOrgOrderConfig/SE').then((response) => {
				if (response.data.code == 0 && response.data.data) {
					...
					this.ruleForm.billingType = response.data.data.billingType
					if (!this.ruleForm.billingType) {
						this.ruleForm.billingType = 'MBL'
					}
				} else {
					this.ruleForm.goodsType = '普货'
					this.ruleForm.containerMethod = '整箱'
