<template>
	<div class="orderEditPage">
		<el-form v-loading="vLoad" :model="ruleForm" :inline="true" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
			...
			<el-row>
              ...
              <el-select slot="suffix"  clearable @change="onChangeDepartureStation"  remote :filter-method="changeDepartureStation" filterable  :loading="loading" v-model="ruleForm.departureStation"
				...
				<!--			<el-select slot="suffix" v-model="ruleForm.departureStation" @change="onChangeDepartureStation" @visible-change="blurDepartureStation" :filter-method="changeDepartureStation" clearable filterable placeholder="请选择" style="width:198px;margin-right: -5px;">
				...
				<el-col class="elementWidth">
					<el-form-item prop="billingType">
						<el-input auto-complete="off" class="widthWithFourWithXing" style="width:295px;margin-right: 15px;">
							...
							<el-select slot="suffix" v-model="ruleForm.billingType" filterable placeholder="请选择" style="width:198px;margin-right: -5px;">
				...
				<el-col class="elementWidth">
					<el-form-item>
						<el-input auto-complete="off" style="width:295px;margin-right: 15px;">
							...
							<el-select slot="suffix" v-model="ruleForm.billingMethod" clearable filterable placeholder="请选择" style="width:198px;margin-right: -5px;">
				...
				<el-col class="elementWidth">
					<el-form-item prop="containerMethod">
						<el-input auto-complete="off" class="widthWithFourWithXing" style="width:295px;margin-right: 15px;">
							...
							<el-select slot="suffix" v-model="ruleForm.containerMethod" filterable placeholder="请选择" style="width:198px;margin-right: -5px;">
			...
			<div style="padding:0px 0px 0px 30px">
				...
				<el-row v-show="showFlagConAndShipper&&ifHaveHblNumber">
					<el-col class="elementWidth" style="margin-top: 10px;margin-left: 10px;margin-bottom: 10px;">
						<font style="font-size: medium;color: red;">当前业务已制单，若制单信息有变，请到制单页面调整</font>
					</el-col>
				</el-row>
									...
									<el-form-item prop="apCode" label-width="10px">
										<el-input class="showWordLimit" auto-complete="off" style="width:335px;" clearable show-word-limit>
											...
											<el-select slot="suffix" :disabled="!ruleForm.containerLoadService" v-model="ruleForm.containerLoadWarehouseId" @change="changeScWarehouse" clearable placeholder="请选择" style="width:234px;margin-right: -5px;">
												<el-option v-for="(item,index) in scWarehouseList" :key="item.warehouseId" :label="item.warehouseNameCn" :value="item.warehouseId">
...
/*				if (val) {
        ...
        this.search("/sc/portMaintenance/search/", val, function(data){
			...
			onChangeDepartureStation(val) {
				this.ruleForm.containerLoadWarehouseId = null;
				//加载堆场/仓库
				let a = {
					apCode: val,
					businessScope: 'SE'
				}
				this.$axios.get2('/sc/scWarehouse/list', a).then((response) => {
					this.scWarehouseList = response.data.data;
				})
			},
			changeScWarehouse(val) {
				let warehouseAddressGps = "";
				if (!this.ruleForm.containerLoadAddressCn) {
					this.scWarehouseList.forEach(s => {
			...
			changeActiveOperationName(val) {
				if (val == 'TG') {
					if (!this.ruleForm.containerPickupService) {
						...
						this.ruleForm.containerPickupDate = null
						this.ruleForm.containerPickupAddress = ''
						this.ruleForm.containerArrivalDate = null
						this.ruleForm.containerArrivalAddress = ''
						this.ruleForm.containerReturnDate = null
					...
					if (!this.ruleForm.containerLoadService) {
						...
						this.ruleForm.containerLoadDate = null
					...
					if (!this.ruleForm.arrivalCustomsClearanceService) {
						...
						this.ruleForm.arrivalCustomsInspectionDate = null
						this.ruleForm.arrivalCustomsClearanceDate = null
            ...
            if(this.ruleForm.transitStation){
                this.transitStations.push(ts);
						...
						this.frow.ruleForm = this.ruleForm
						this.frow.ruleFormOldStr = JSON.stringify(response.data.data)
						let containerLoadWarehouseId = this.ruleForm.containerLoadWarehouseId;
						this.onChangeDepartureStation(this.ruleForm.departureStation);
						this.ruleForm.containerLoadWarehouseId = containerLoadWarehouseId;
						/*this.$axios.get('/sc/portMaintenance').then((res) => {
							this.seaPorts = res.data.data;
							this.departureStations = this.seaPorts.slice(0, 10)
							this.transitStations = this.seaPorts.slice(0, 10)
							this.arrivalStations = this.seaPorts.slice(0, 10)
		...
		created() {
			...
			this.$axios.get('/sc/waybillPrint/view/' + this.frow.orderId + '/HBL').then(function(response) {
				if (response.data.code == 0 && response.data.data.waybillPrintId) {
					this.ifHaveHblNumber = true
				}
			}.bind(this));
			//查询用户设置小数位
