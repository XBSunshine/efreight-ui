<template>
	<el-dialog title="海运出口订单 - 编辑" :fullscreen="ifFullscreen" width="1300px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" :show-close="!ifFullscreen" :style="style">
...
<script>
	const OrderInfo = resolve => require(['@/views/sc/se/order/main/component_edit/order_edit_orderinfo.vue'], resolve)
	...
	export default {
		...
		created() {
			...
			this.$axios.get('/hrs/permission/' + 307).then((response) => {
			...
			this.$axios.get('/hrs/permission/' + 308).then((response) => {
				if (response.data.code == 0) {
			...
			this.$axios.get('/hrs/permission/' + 309).then((response) => {
				if (response.data.code == 0) {
