...
<script>
  import columns from '../json/se_order_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_se_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.findByPage();
     ...
     sureSave(){
       ...
       if(this.multipleSelection&&this.multipleSelection.length>0){
         ...
         localStorage.removeItem(userId+'_se_list_column_width');//清除掉页面拖动字段宽度的缓存
         /*localStorage.setItem(userId+'_se_list_column',JSON.stringify(this.multipleSelection));
         this.handleClose();*/
       }else{
    ...
    sortBykey(ary, key) {
      return ary.sort(function (a, b) {
		...
		created() {
      //let  userId = window.localStorage.getItem('userId');
      let seInfo = JSON.stringify(columns.seInfo);
      this.oldTableColumns =seInfo;
      ...
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.se_list_column = response.data.data;
          }.bind(this));

      if(this.se_list_column){
          this.tableColumns = JSON.parse(seInfo);
      }else{
        this.tableColumns = JSON.parse(seInfo);
        this.tableColumns =  this.sortBykey(this.tableColumns, 'index');
      }
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  se_list_column_width = window.localStorage.getItem(userId+'_se_list_column_width');
      //从数据库查询设置信息
      let pageName = '订单操作管理/海运出口/SE订单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              ...
              //如果se_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(se_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(se_list_column_width);
              ...
              if(this.se_list_column && this.se_list_column.length>0){
                  let loaclJsonArray = this.se_list_column;
      ...
      /*if(se_list_column){
        let loaclJsonArray = JSON.parse(se_list_column);
        if(loaclJsonArray&&loaclJsonArray.length>0){
          let liftNo = [];
          let lift = [];
           for(var i=0;i<this.tableColumns.length;i++){
