		...
		<div class="orderEditPage" style="width: 1280px;">
			<el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
				<div style="margin-top: -15px;margin-bottom: 20px;">
					<span style="font-size: large;margin-bottom: 20px;">海运进口订单&nbsp;-&nbsp;新增</span>
				...
				<el-row>
					<el-col class="elementWidth">
						<el-form-item prop="orderCode">
							<el-input v-model="ruleForm.orderCode" @input="ruleForm.orderCode=ruleForm.orderCode.toUpperCase()" auto-complete="off" disabled style="width:240px;margin-right: 15px;">
      ...
      this.$axios.get('/hrs/userWorkgroup/selectWorkgroup?businessScope=SI').then(function(response) {
          ...
          if(userWorkgroupDetailList != null && userWorkgroupDetailList.length > 0){
			...
			this.$axios.get('/sc/vScCategory/放货方式').then((response) => {
				...
				if (this.dischargingMethods.length != 0) {
					this.ruleForm.dischargingMethod = this.dischargingMethods[0].paramText
				}
			})
			...
			//查询签约公司默认配置信息
			this.$axios.get('/hrs/org/getOrgOrderConfig/SI').then((response) => {
