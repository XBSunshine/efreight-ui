...
<script>
  import columns from './si_order_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_si_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.findByPage();
     ...
     sureSave(){
       ...
       if(this.multipleSelection&&this.multipleSelection.length>0){
         ...
         localStorage.removeItem(userId+'_si_list_column_width');//清除掉页面拖动字段宽度的缓存
         /*localStorage.setItem(userId+'_si_list_column',JSON.stringify(this.multipleSelection));
         this.handleClose();*/
       }else{
    ...
    sortBykey(ary, key) {
      return ary.sort(function (a, b) {
		...
		created() {
      //let  userId = window.localStorage.getItem('userId');
      let siInfo = JSON.stringify(columns.siInfo);
      this.oldTableColumns =siInfo;
      ...
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.si_list_column = response.data.data;
          }.bind(this));
      if(this.si_list_column){
          this.tableColumns = JSON.parse(siInfo);
      }else{
        this.tableColumns = JSON.parse(siInfo);
        this.tableColumns =  this.sortBykey(this.tableColumns, 'index');
      }
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  si_list_column_width = window.localStorage.getItem(userId+'_si_list_column_width');
      //从数据库查询设置信息
      let pageName = '订单操作管理/海运进口/SI订单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              ...
              //如果si_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(si_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(si_list_column_width);
              ...
              if(this.si_list_column && this.si_list_column.length>0){
                  let loaclJsonArray = this.si_list_column;
...
/*      if(si_list_column){
        let loaclJsonArray = JSON.parse(si_list_column);
        if(loaclJsonArray&&loaclJsonArray.length>0){
          let liftNo = [];
          let lift = [];
           for(var i=0;i<this.tableColumns.length;i++){
