<template>
	<el-dialog title="" :fullscreen="ifFullscreen" width="1300px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" :style="style" :show-close="!ifFullscreen">
		<div v-loading="vLoad" class="lcOrderEditPage" style="width: 1280px;">
			<el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
				<div style="margin-top: -15px;margin-bottom: 20px;">
					<span style="font-size: large;margin-bottom: 20px;">陆运订单&nbsp;-&nbsp;复制</span>
          ...
          <el-col class="elementWidth">
            <el-form-item>
              <el-input auto-complete="off" style="width:295px;margin-right: 15px;" clearable>
				...
				<el-row>
					<el-form-item>
						<el-button :loading="loading" type="primary" size="small" @click="submitForm('ruleForm')">复制新增</el-button>
					</el-form-item>
				</el-row>
			</el-form>
	...
	export default {
      ...
      setWorkgroup(vId) {
          ...
          if(obj){
			...
			emptyData() {
				...
				this.ruleForm.orderCodeVl = ''
				this.ruleForm.driverNumber = ''
				this.ruleForm.driverName = ''
				this.ruleForm.driverTel = ''
			},
      searchCity(val){
      ...
      search(url, val, callback){
		...
		created() {
			...
			this.$axios.get('/hrs/permission/' + 425).then(function(response) {
				if (response.data.code == 0) {
