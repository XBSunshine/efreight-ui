<template>
	<el-dialog title="" :fullscreen="ifFullscreen" width="1300px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" :style="style" :show-close="!ifFullscreen">
		<div class="lcOrderEditPage" style="width: 1280px;">
			<el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
				<div style="margin-top: -15px;margin-bottom: 20px;">
					<span style="font-size: large;margin-bottom: 20px;">陆运订单&nbsp;-&nbsp;新增</span>
            ...
            <el-form-item>
              <el-input auto-complete="off" style="width:295px;margin-right: 15px;" clearable>
				...
				<el-row>
					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input maxlength="20" class="widthWithThree" v-model="ruleForm.driverNumber" style="width:295px;margin-right: 15px;">
								<template slot="prepend">
									<span>车牌号</span>
								</template>
							</el-input>
	...
	export default {
		...
		methods: {
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.loading = true
						this.$axios.post2('/sc/lcOrder', this.ruleForm)
							.then((response) => {
      ...
      setWorkgroup(vId) {
          ...
          if(obj){
			...
			setPlanChargeWeight() {
				...
				if (this.ruleForm.planVolume != null && this.ruleForm.planVolume !== '' && parseFloat(this.ruleForm.planVolume + '') !=
					0 && this.ruleForm.planWeight != null && this.ruleForm.planWeight !== '') {
      ...
      search(url, val, callback){
		...
		created() {
			...
			this.$axios.get('/hrs/permission/' + 416).then(function(response) {
				if (response.data.code == 0) {
      ...
      this.$axios.get('/hrs/userWorkgroup/selectWorkgroup?businessScope=LC').then(function(response) {
        ...
        //
			...
			//查询签约公司默认配置信息
			this.$axios.get('/hrs/org/getOrgOrderConfig/LC').then((response) => {
				if (response.data.code == 0 && response.data.data) {
					this.ruleForm.goodsType = response.data.data.goodsType
					if (!this.ruleForm.goodsType) {
						this.ruleForm.goodsType = '普货'
					}
					this.ruleForm.shippingMethod = response.data.data.shippingMethod
					if (!this.ruleForm.shippingMethod) {
						this.ruleForm.shippingMethod = '整车'
					}
				} else {
					this.ruleForm.goodsType = '普货'
					this.ruleForm.shippingMethod = '整车'
