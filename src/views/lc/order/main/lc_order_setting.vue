...
<script>
  import columns from '../../order/json/lc_order_column.json'
	...
	export default {
        ...
        localStorage.removeItem(userId+'_lc_order_list_column_width');//清除掉页面拖动字段宽度的缓存
				this.findByPage();
		...
		created() {
			let lcInfo = JSON.stringify(columns.lcInfo);
			this.oldTableColumns = lcInfo;
      this.tableColumns = JSON.parse(lcInfo);
      this.tableColumns = this.sortBykey(this.tableColumns, 'index');
		},
		mounted() {
			let userId = window.localStorage.getItem('userId');
      let  lc_order_list_column_width = window.localStorage.getItem(userId+'_lc_order_list_column_width');
        //从数据库查询设置信息
        let pageName = '订单操作管理/国内运输/运输订单';
        this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
            .then(function(response) {
                this.lc_order_list_column = response.data.data;

                //如果lc_order_list_column_width不为空，则说明在页面拖动过字段的宽度
                if(lc_order_list_column_width){
                    let loaclWidthJsonArray = JSON.parse(lc_order_list_column_width);
                ...
                if(this.lc_order_list_column && this.lc_order_list_column.length>0){
                    let loaclJsonArray = this.lc_order_list_column;
