...
<script>
	import columns from './prm_report_receivable_age_list_column.json'
	...
	export default {
        ...
        handleClose() {
            ...
            localStorage.removeItem(userId + '_prm_report_receivable_age_list_column_width'); //清除掉页面拖动字段宽度的缓存
            this.findByPage();
		...
		created() {
			let prmReportReceivableAgeInfo = JSON.stringify(columns.prmReportReceivableAgeInfo);
			this.oldTableColumns = prmReportReceivableAgeInfo;
			this.tableColumns = JSON.parse(prmReportReceivableAgeInfo);
			this.tableColumns = this.sortBykey(this.tableColumns, 'index')
			//this.setLabel(this.tableColumns)
		},
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let prm_report_receivable_age_list_column_width = window.localStorage.getItem(userId + '_prm_report_receivable_age_list_column_width');
			//从数据库查询设置信息
			...
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
					...
					if (prm_report_receivable_age_list_column_width) {
						let loaclWidthJsonArray = JSON.parse(prm_report_receivable_age_list_column_width);
							...
							for (var i = 0; i < this.tableColumns.length; i++) {
								...
								for (var j = 0; j < loaclJsonArray.length; j++) {
									if (this.tableColumns[i].prop == loaclJsonArray[j].prop) {
										...
										if (prm_report_receivable_age_list_column_width) {} else {
