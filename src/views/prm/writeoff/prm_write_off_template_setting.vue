...
export default {
  ...
  methods: {
    ...
    sureSave() {
      ...
      if (this.multipleSelection && this.multipleSelection.length > 0) {
        ...
        localStorage.removeItem(userId + '_prm_write_off_column_width'); //清除掉页面拖动字段宽度的缓存
      } else {
  ...
  created() {
    let afInfo = JSON.stringify(columns.columns);
  ...
  mounted() {
    let userId = window.localStorage.getItem('userId');
    let af_list_column_width = window.localStorage.getItem(userId + '_prm_write_off_column_width');
    //从数据库查询设置信息
    this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + this.pageName)
      .then(function (response) {
        this.af_list_column = response.data.data;
