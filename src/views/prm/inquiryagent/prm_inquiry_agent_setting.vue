...
<script>
	import columns from './prm_inquiry_agent_column.json'
	...
	export default {
        ...
        localStorage.removeItem(userId+'_inquiry_agent_list_column_width');//清除掉页面拖动字段宽度的缓存
				this.findByPage();
		...
		created() {
			let inquiryAgent = JSON.stringify(columns.inquiryAgent);
			this.oldTableColumns = inquiryAgent;
      this.tableColumns = JSON.parse(inquiryAgent);
      this.tableColumns = this.sortBykey(this.tableColumns, 'index');
		},
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let inquiry_agent_list_column_width = window.localStorage.getItem(userId + '_inquiry_agent_list_column_width');
      //从数据库查询设置信息
      let pageName = '客商资料管理/询盘代理';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.inquiry_agent_list_column = response.data.data;

              //如果inquiry_agent_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(inquiry_agent_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(inquiry_agent_list_column_width);
              ...
              if(this.inquiry_agent_list_column && this.inquiry_agent_list_column.length>0){
                  let loaclJsonArray = this.inquiry_agent_list_column;
