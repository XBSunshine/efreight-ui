<template>
	<el-dialog title="询盘代理 - 新增" width="995px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
	...
	export default {
		...
		methods: {
			...
			changeDepartureStation(val) {
				if (val) {
					this.airportOptions = this.airportOptions2.filter(port => port.code.indexOf(val.toUpperCase()) > -1 || port.label.indexOf(
						val.toUpperCase()) > -1).slice(0, 30)
				} else {
					this.airportOptions = this.airportOptions2.slice(0, 30)
				}
			},
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$axios.post('/prm/inquiryAgent/doSave', params)
							.then(function(response) {
