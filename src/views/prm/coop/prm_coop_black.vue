<template>
	<el-dialog title="黑名单录入" width="800px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
        ...
        <el-col class="elementWidth">
          <el-form-item prop="reason" :rules="[{required:true,message:'请录入黑名单原因',trigger:['blur','change']}]" label-width="15px">
            <el-input maxlength="100" show-word-limit v-model="form.reason" style="width: 450px;">
              <template slot="prepend">录入黑名单原因</template>
            </el-input>
          </el-form-item>
        </el-col>
			</el-row>
	...
	export default {
		...
		methods: {
			...
			submitForm() {
				...
				if(this.form.reason == '') {
					this.openError("请录入黑名单原因")
					return
				}

				this.$axios.put('/prm/coop/modifyBlackWhiteValid/0', this.form)
