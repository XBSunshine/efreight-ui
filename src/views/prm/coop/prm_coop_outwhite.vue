<template>
	<el-dialog title="白名单 - 移出" width="800px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
        ...
        <el-col class="elementWidth">
          <el-form-item prop="outreason" :rules="[{required:true,message:'请录入移出白名单原因',trigger:['blur','change']}]" label-width="15px">
            <el-input v-model="form.outreason" maxlength="100" show-word-limit style="width: 450px;">
              <template slot="prepend">移出白名单原因</template>
            </el-input>
          </el-form-item>
        </el-col>
			</el-row>
...
<script>
	export default {
		...
		created: function() {
			...
			if(this.frow.whitelist_reason == null) {
				this.form.reason = "";
			} else {
				this.form.reason = this.frow.whitelist_reason;
			}
		},
		...
		methods: {
			...
			submitForm() {
				...
				if(this.form.outreason == '') {
					this.openError("请录入移出白名单原因")
					return
				}
				this.$axios.put('/prm/coop/modifyOutBlackWhiteValid/1', this.form)
