<template>
	<el-dialog title="锁定客商资料" width="800px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
        ...
        <el-col class="elementWidth">
          <el-form-item :rules="[{required:true,message:'请录入锁定原因',trigger:['blur','change']}]" label-width="15px">
            <el-input v-model="form.reason" maxlength="100" show-word-limit style="width: 450px;">
              <template slot="prepend">锁定原因</template>
            </el-input>
          </el-form-item>
        </el-col>
			</el-row>
	...
	export default {
		...
		methods: {
			...
			submitForm() {
				...
				let reason = encodeURIComponent(this.form.reason)
				this.$axios.put('/prm/coop/lockOrUnlock/0', this.form)
