<template>
	<el-dialog title="黑名单 - 移出" width="800px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
        ...
        <el-col class="elementWidth">
          <el-form-item prop="outreason" :rules="[{required:true,message:'请录入移出黑名单原因',trigger:['blur','change']}]" label-width="15px">
            <el-input maxlength="100" show-word-limit v-model="form.outreason" style="width: 450px;">
              <template slot="prepend">移出黑名单原因</template>
            </el-input>
          </el-form-item>
        </el-col>
			</el-row>
...
<script>
	export default {
		...
		created: function() {
			...
			if(this.frow.blacklist_reason == null) {
				this.form.reason = "";
			} else {
				this.form.reason = this.frow.blacklist_reason;
			}
		},
		...
		methods: {
			...
			submitForm() {
				...
				if(this.form.outreason == '') {
					this.openError("请录入移出黑名单原因")
					return
				}

				this.$axios.put('/prm/coop/modifyOutBlackWhiteValid/0', this.form)
