...
<script>
  import columns from './prm_coop_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_prm_coop_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.queryCoopList();
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  prm_coop_list_column_width = window.localStorage.getItem(userId+'_prm_coop_list_column_width');
      //从数据库查询设置信息
      let pageName = '客商资料管理/客商资料';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.prm_coop_list_column = response.data.data;

              //如果prm_coop_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(prm_coop_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(prm_coop_list_column_width);
              ...
              if(this.prm_coop_list_column && this.prm_coop_list_column.length>0){
                  let loaclJsonArray = this.prm_coop_list_column;
