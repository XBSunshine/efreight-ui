...
<script>
	import columns from './prm_bill_settle_column.json'
	...
	export default {
        ...
        handleClose() {
            ...
            localStorage.removeItem(userId + '_prm_bill_settle_column_width'); //清除掉页面拖动字段宽度的缓存
		...
		created() {
			let prmBillSettleInfo = JSON.stringify(columns.prmBillSettleInfo);
			this.oldTableColumns = prmBillSettleInfo;
			this.tableColumns = JSON.parse(prmBillSettleInfo);
			this.tableColumns = this.sortBykey(this.tableColumns, 'index')
		},
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let prm_bill_settle_column_width = window.localStorage.getItem(userId + '_prm_bill_settle_column_width');
			//从数据库查询设置信息
			...
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
					...
					if (prm_bill_settle_column_width) {
						let loaclWidthJsonArray = JSON.parse(prm_bill_settle_column_width);
							...
							for (var i = 0; i < this.tableColumns.length; i++) {
								...
								for (var j = 0; j < loaclJsonArray.length; j++) {
									if (this.tableColumns[i].prop == loaclJsonArray[j].prop) {
										...
										if (prm_bill_settle_column_width) {} else {
