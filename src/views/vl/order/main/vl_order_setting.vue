	...
	export default {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_vl_order_list_column_width'); //清除掉页面拖动字段宽度的缓存
				this.findByPage();
		...
		created() {
			let vlInfo = JSON.stringify(columns.vlInfo);
			this.oldTableColumns = vlInfo;
			this.tableColumns = JSON.parse(vlInfo);
			this.tableColumns = this.sortBykey(this.tableColumns, 'index');
		},
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let vl_order_list_column_width = window.localStorage.getItem(userId + '_vl_order_list_column_width');
			//从数据库查询设置信息
			let pageName = '订单操作管理/国内运输/车辆配载';
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
					this.vl_order_list_column = response.data.data;

					//如果vl_order_list_column_width不为空，则说明在页面拖动过字段的宽度
					if (vl_order_list_column_width) {
						let loaclWidthJsonArray = JSON.parse(vl_order_list_column_width);
					...
					if (this.vl_order_list_column && this.vl_order_list_column.length > 0) {
						let loaclJsonArray = this.vl_order_list_column;
							...
							for (var i = 0; i < this.tableColumns.length; i++) {
								...
								for (var j = 0; j < loaclJsonArray.length; j++) {
									if (this.tableColumns[i].prop == loaclJsonArray[j].prop) {
										...
										if (vl_order_list_column_width) {} else {
