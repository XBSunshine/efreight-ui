<template>
	<el-dialog title="派车单 - 完成订单" :fullscreen="ifFullscreen" width="1300px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" :style="style" :show-close="!ifFullscreen">
		<div v-loading="vLoad" class="vlOrderEditPage" style="width: 1280px;">
			<el-form disabled :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
				...
				<el-row>
					<el-col class="elementWidth">
						<el-form-item>
							<el-input class="widthWithThree" style="width:295px;margin-right: 15px;">
								...
								<el-select @change="changeTruck" slot="suffix" v-model="ruleForm.truckId" placeholder="请选择" style="width:198px;margin-right: -5px;">
					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input v-model="ruleForm.driverTel" style="width:615px;">
								<template slot="prepend">
									<span>司机电话</span>
								</template>
							</el-input>
				...
				<el-table :data="ruleForm.detailOrderList" stripe border style="width:1255px" :header-cell-style="headerCellStyle">
					...
					<el-table-column align="center" label="分摊成本">
						...
						<el-table-column align="center" label="分摊金额" width="160">
							<template slot-scope="scope">
								<span v-if="scope.row.costAmountStr&&scope.row.costAmountStr.indexOf('CNY')==-1" style="color: red;">{{scope.row.costAmountStr}}</span>
			...
			<el-row>
				<el-button style="margin-left: 10px;" :loading="loading" type="primary" size="small" @click="submitForm('ruleForm')">完成</el-button>
			</el-row>
		...
		methods: {
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.loading = true
						this.$axios.put('/sc/vlOrder/finish', this.ruleForm)
							.then((response) => {
			...
			view() {
				this.vLoad = true
				this.$axios.get('/sc/vlOrder/viewForFinishOrder/' + this.frow.orderId).then((response) => {
					if (response.data.code == 0) {
			...
			headerCellStyle({
				...
				if ((rowIndex == 0 && columnIndex == 3) || (rowIndex == 1 && columnIndex == 0) || (rowIndex == 1 && columnIndex == 1)) {
					return 'background:#fee1e3'
				}
				if ((rowIndex == 0 && columnIndex == 4) || (rowIndex == 1 && columnIndex == 2) || (rowIndex == 1 && columnIndex == 3) || (rowIndex == 1 && columnIndex == 4) || (rowIndex == 1 && columnIndex == 5)) {
					return 'background:#FDF5E6'
				}
				if ((rowIndex == 0 && columnIndex == 5) || (rowIndex == 1 && columnIndex == 6) || (rowIndex == 1 && columnIndex == 7) || (rowIndex == 1 && columnIndex == 8) || (rowIndex == 1 && columnIndex == 9)) {
