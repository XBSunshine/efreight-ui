<template>
  <el-dialog title="其他业务订单 - 集装箱量" width="1050px" :visible.sync="visible" :close-on-press-escape="false"
             :close-on-click-modal="false" :before-close="handleClose" append-to-body style="top:-80px;">
    ...
    <el-table :data="data" style="width: 1050px;" class="orderEditContainerPage">
      <el-table-column label="箱号" align="center" width="200">
        <template slot-scope="scope">
          <el-input size="small" v-model="scope.row.containerNumber" clearable
                    @input="scope.row.containerNumber=scope.row.containerNumber.toUpperCase()"></el-input>
        </template>
      </el-table-column>
      ...
      <el-table-column label="尺寸" align="center" width="80">
        <template slot-scope="scope">
          <el-select v-model="scope.row.containerSize" clearable placeholder="选择尺寸" @change="countTotal">
            <el-option v-for="(item,index) in containerSizes" :key="index" :label="item.paramText"
      ...
      <el-table-column label="箱型" align="center" width="80">
        <template slot-scope="scope">
          <el-select v-model="scope.row.containerCode" clearable placeholder="选择箱型" @change="countTotal">
            <el-option v-for="(item,index) in containerCodes" :key="index" :label="item.paramText"
                       :value="item.paramText">
            </el-option>
          </el-select>
        </template>
      ...
      <el-table-column label="件数" align="center">
        <template slot-scope="scope">
          <el-input size="small" v-model="scope.row.pieces" maxlength="6" clearable
                    @input="checkInt(scope.row,'pieces')" @change="countTotal"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="毛重" align="center">
        <template slot-scope="scope">
          <el-input size="small" v-model="scope.row.weight" clearable @input="checkFloat(scope.row,'weight')"
                    @change="countTotal"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="体积" align="center">
        <template slot-scope="scope">
          <el-input size="small" v-model="scope.row.volume" clearable @input="checkFloat(scope.row,'volume')"
                    @change="countTotal"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="VGM重量" align="center">
        <template slot-scope="scope">
          <el-input size="small" v-model="scope.row.weightVgm" clearable @input="checkFloat(scope.row,'weightVgm')"
                    @change="countTotal"></el-input>
        </template>
      </el-table-column>
      <el-table-column fixed width="60" align="center">
        <template slot="header">
          <i class="iconfont el-icon-myyuanquanjiahao" @click="doAdd"
             style="cursor: pointer;size: b5;color: #409EFF;"></i>
        </template>
        <template slot-scope="scope">
          <i class="iconfont el-icon-myjianhao" @click="doDelete(scope.$index,scope.row)"
             style="cursor: pointer;size: b5;color: #409EFF;"></i>
        </template>
      </el-table-column>
    </el-table>
    <div
      style="width: 1010px;margin-left:10px;margin-top:20px;border-bottom: 15px solid #EBEEF5;border-left: 15px solid #EBEEF5;border-right: 15px solid #EBEEF5;border-radius: 10px 10px;">
      <div style="background-color: #EBEEF5;height: 35px;width: auto;">
        <font style="float:left;margin-top: 10px;">快速填写</font>
      </div>
      <el-input type="textarea" rows="6" cols="35" v-model="inputSize" placeholder="样例：20GP*5;40HQ*1"
                auto-complete="off">
      </el-input>
      <el-button style="float:left;position: relative;left:810px;top:-45px;" round @click="emptyInputMany">清空
      </el-button>
    ...
    <div style="margin-left: 10px;margin-bottom: 10px;">
      <el-button type="primary" size="small" @click="empty" style="background: #FFFFFF;color: #409EFF;" plain>清空
      </el-button>
