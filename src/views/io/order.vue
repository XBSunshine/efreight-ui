<template>
	<div class="warp-main">
		<el-form :inline="false" :model="query" label-width="10px" class="orderListPage">
			<div ref="io_order_header">
					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input style="width:200px;">
								...
								<el-select slot="suffix" v-model="query.businessMethod" clearable placeholder="请选择" style="width:131px;margin-right: -5px;">
					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input style="width:204px;">
								<template slot="prepend">业务日期</template>
								<el-date-picker slot="suffix" v-model="query.businessDateStart" clearable type="date" value-format="yyyy-MM-dd" placeholder="开始" style="width: 135px;margin-right: -5px;">
								</el-date-picker>
							</el-input>
							<span>-</span>
							<el-date-picker v-model="query.businessDateEnd" clearable type="date" value-format="yyyy-MM-dd" placeholder="结束" style="width: 135px;">
							</el-date-picker>
						</el-form-item>
					</el-col>
					<el-col class="elementWidth">
				...
				<el-row v-show='showFlag'>
					...
					<el-col class="elementWidth">
						<el-form-item class="widthWithThree">
		...
		<div class="pagination" ref="io_order_footer">
...
<script>
	...
	import setVisibleVue from './order/main/io_order_setting.vue'
	...
	export default {
		data() {
			return {
				...
				query: {
					...
					coopName: '',
					businessMethod: '',
					businessDateStart: this.getDateTime(),
					businessDateEnd: '',
					orderCode: '',
					orderCodeAssociated: '',
					departureStation: '',
		...
		methods: {
			...
			setHeight() {
				this.$nextTick(() => {
					let io_order_header = this.$refs.io_order_header.offsetHeight;
					let io_order_footer = this.$refs.io_order_footer.offsetHeight;
					let heightS = window.innerHeight - 90 - io_order_header - io_order_footer;
			...
			cellWidth(newWidth, oldWidth, column, event) {
				...
				if (arrayC && arrayC.length > 0) {
					...
					localStorage.setItem(userId + '_io_order_list_column_width', JSON.stringify(arrayC))
				}
			},
			setting() {
			...
			formatter_column(row, column) {
				...
				} else if (column.property === 'planPieces'||column.property === 'planWeight' || column.property === 'planVolume' || column.property === 'planChargeWeight') {
        ...
        this.query.orderPermission = orderPermission;
				...
				this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
					.then(function(response) {
						let io_list_column = response.data.data;
						if (io_list_column && io_list_column.length > 0) {
							this.tableColumns = this.sortBykey(io_list_column, 'index');
						} else {
							this.tableColumns = this.sortBykey(columns.ioInfo, 'index');
						}
						this.loading = true
						this.$axios.get2("/sc/ioOrder?size=" + this.pageConf.pageSize + "&current=" + this.pageConf.pageCode, this.query)
							.then((response) => {
						...
						this.$axios.get2("/sc/ioOrder/total", this.query)
							.then(function(response) {
			...
			findByPage2(current) {
				this.$axios.get2("/sc/ioOrder?size=" + this.pageConf.pageSize + "&current=" + current, this.query)
					.then((response) => {
						this.loading = false
						this.data = response.data.data.records;
						this.pageConf.totalPage = response.data.data.total;
			...
			exportExcelList() {
				...
				this.$confirm('是否根据结果字段导出数据？', '提示', {
					...
					this.query.columnStrs = JSON.stringify(columns.ioInfo);
					this.exportExcelSure();
				});
			},
			exportExcelSure() {
				this.$axios.post3('/sc/ioOrder/exportExcel', this.query)
					.then(function(response) {
						...
						downloadElement.download = 'IO订单' + '.xls'; // 下载后文件名
        ...
        if (_month === 0) {
					...
					for (let i = 0; i < loaclJsonArray.length; i++) {
						if (this.total != null) {
							if (loaclJsonArray[i].prop === "planPieces") {
								sums[i + 1] = this.total.planPiecesStr
							}
			...
			init() {
				...
				if (buttonPermission.some(item => item == 'io_order_twoCode')) {
					this.permissionButtonForTwoCode = true
				}
				if (buttonInfo.indexOf('io_order_twoCode_view') > -1) {
					this.permissionButtonForViewTwoCode = true
				}
				if (buttonInfo.indexOf('io_order_twoCode_share') > -1) {
					this.permissionButtonForShareTwoCode = true
				}
				if (buttonPermission.some(item => item == 'io_order_specialHandle')) {
					this.permissionButtonForSpecialHandle = true
				}
				if (buttonInfo.indexOf('io_order_specialHandle_copy') > -1) {
					this.permissionButtonForSpecialHandleForceStop = true
				}
				if (buttonInfo.indexOf('io_order_specialHandle_forceStop') > -1) {
					this.permissionButtonForSpecialHandleCopy = true
				}
				if (buttonInfo.indexOf('io_order_export') > -1) {
		...
		created() {
			this.$axios.get('/hrs/org/getOrderFinanceLockView/IO').then(function(response) {
