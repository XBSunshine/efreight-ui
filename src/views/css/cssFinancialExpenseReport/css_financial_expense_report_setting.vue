...
<el-dialog title="设置字段" width="580px" :visible.sync="visible" append-to-body :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" style="top:-80px;">
	...
	<div class="css_f_e_r_colmun_setting">
...
<script>
  import columns from './css_financial_expense_report_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_css_financial_expense_report_column_width');//清除掉页面拖动字段宽度的缓存
      	this.findByPage();
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  column_width = window.localStorage.getItem(userId+'_css_financial_expense_report_column_width');
      //从数据库查询设置信息
      ...
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              ...
              //如果column_width不为空，则说明在页面拖动过字段的宽度
              if(column_width){
                  let loaclWidthJsonArray = JSON.parse(column_width);
...
<style scope>
	.css_f_e_r_colmun_setting .el-table .warning-row2 {
		background: oldlace;
		color: #409EFF;
		font-weight: bold !important;
	}

	.css_f_e_r_colmun_setting .el-table .warning-row1 {
		background: oldlace;
		font-weight: bold !important;
	}

	.css_f_e_r_colmun_setting .el-table .warning-row0 {
		color: #409EFF;
	}

	.css_f_e_r_colmun_setting .el-table .cell {
		white-space: pre-line !important;
		/*保留换行符*/
	}

	.css_f_e_r_colmun_setting .el-table .hidden_row {
		display: none;
		/*保留换行符*/
	}
  .awbNumber{
