...
<script>
	import columns from './css_report_no_writeoff_column.json'
	...
	export default {
		...
		data() {
			return {
				...
				multipleSelection: [],
				oldTableColumns: '',
				css_report_no_writeoff_list_column: []
			}
		},
		inject: ['queryList'],
		provide() {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_css_report_no_writeoff_column_width'); //清除掉页面拖动字段宽度的缓存
				this.queryList();
		...
		created() {
			let info = JSON.stringify(columns.css_report_no_writeoff);
		...
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let css_report_no_writeoff_column_width = window.localStorage.getItem(userId + '_css_report_no_writeoff_column_width');
			//从数据库查询设置信息
			let pageName = '报表统计分析/往来情况查询';
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
					this.css_report_no_writeoff_list_column = response.data.data;
					//如果css_report_no_writeoff_column_width不为空，则说明在页面拖动过字段的宽度
					if (css_report_no_writeoff_column_width) {
						let loaclWidthJsonArray = JSON.parse(css_report_no_writeoff_column_width);
					...
					if (this.css_report_no_writeoff_list_column && this.css_report_no_writeoff_list_column.length > 0) {
						let loaclJsonArray = this.css_report_no_writeoff_list_column;
							...
							for (var i = 0; i < this.tableColumns.length; i++) {
								...
								for (var j = 0; j < loaclJsonArray.length; j++) {
									if (this.tableColumns[i].prop == loaclJsonArray[j].prop) {
										...
										if (css_report_no_writeoff_column_width) {} else {
