...
<script>
  import columns from './css_income_nodebitnote.json'
    ...
    setLabel(tableColumns) {
    	try {
          ...
          if(column.prop == 'plan_charge_weight_count'){
            if ( this.frow.businessScope.startsWith('S')) {
            	column.label = '箱量/计费吨'
            } else{
              column.label = '计重'
            }
          ...
          if(column.prop == 'plan_charge_weight_count_year'){
            if ( this.frow.businessScope.startsWith('S')&&this.frow.containerMethod!='整箱') {
            	column.label = '年度合作箱量'
            } else{
              column.label = '年度合作货量'
            }
