...
<script>
  import columns from './css_workload_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_css_workload_column_width');//清除掉页面拖动字段宽度的缓存
      	this.queryList();
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  css_workload_column_width = window.localStorage.getItem(userId+'_css_workload_column_width');
      //从数据库查询设置信息
      let pageName = '报表统计分析/工作量统计';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.css_workload_list_column = response.data.data;

              //如果css_workload_column_width不为空，则说明在页面拖动过字段的宽度
              if(css_workload_column_width){
                  let loaclWidthJsonArray = JSON.parse(css_workload_column_width);
              ...
              if(this.css_workload_list_column && this.css_workload_list_column.length>0){
                  let loaclJsonArray = this.css_workload_list_column;
