	...
	export default {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_supplier_procurement_detail_column_width'); //清除掉页面拖动字段宽度的缓存
				this.querCach();
			...
			sureSave() {
				...
				if (this.multipleSelection && this.multipleSelection.length > 0) {
					...
					this.$axios.post2('/hrs/user/saveUserPageSet', {
							...
							pageName: '报表统计分析/供应商采购分析/详情/' + this.frow.businessScope
						})
		...
		created() {
			let info = JSON.stringify(columns.supplierDetail);
		...
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let list_column_width = window.localStorage.getItem(userId + '_supplier_procurement_detail_column_width');
			//从数据库查询设置信息
			let pageName = '报表统计分析/供应商采购分析/详情/' + this.frow.businessScope;
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
