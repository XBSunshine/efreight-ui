...
<script>
	import columns from './css_p_report_supplier_procurement.json'
	...
	export default {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_supplier_procurement_list_column_width'); //清除掉页面拖动字段宽度的缓存
				this.findByPage();
		...
		created() {
			let info = JSON.stringify(columns.supplier);
		...
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let list_column_width = window.localStorage.getItem(userId + '_supplier_procurement_list_column_width');
			//从数据库查询设置信息
			let pageName = '报表统计分析/供应商采购分析';
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
