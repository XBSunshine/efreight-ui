					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input style="width:204px;">
								<template slot="prepend">到达日期</template>
          ...
          <el-col class="elementWidth">
            <el-form-item>
              <el-input style="width:210px;">
                ...
                <el-select slot="suffix" v-model="query.productType" clearable placeholder="请选择" style="width:141px;margin-right: -5px;">
                  <el-option v-for="(item,index) in productTypes" :key="index" :label="item.paramText" :value="item.paramText">
					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input class="widthWithThree" v-model="query.arrivalStation" auto-complete="off" clearable style="width:171px;" @input="query.arrivalStation=query.arrivalStation.toUpperCase()">
					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input style="width:210px;">
								...
								<el-select slot="suffix" v-model="query.costRecorded" placeholder="请选择" style="width:141px;margin-right: -5px;">
									<el-option label="全部" value=""></el-option>
									<el-option label="是" value="true"></el-option>
									<el-option label="否" value="false"></el-option>
								</el-select>
					...
					<el-col class="elementWidth" v-show="orderFinanceLockViewFlag">
						<el-form-item>
							<el-input style="width:204px;">
								...
								<el-select slot="suffix" v-model="query.orderStatus" clearable placeholder="请选择" style="width:135px;margin-right: -5px;">
...
<!--										<el-dropdown-item v-if="permissionButtonForShareTwoCode" command="deliveryNotice">送货通知码</el-dropdown-item>-->
		...
		<div class="pagination" ref="ti_order_footer">
...
<script>
	...
	import columns from './order/main/ti_order_column.json'
	import setVisibleVue from './order/main/ti_order_setting'
	...
	export default {
      ...
      sortCostRecorded(a, b) {
          return a.costRecordedForSort - b.costRecordedForSort
			...
			setHeight() {
				this.$nextTick(() => {
					let order_header = this.$refs.ti_order_header.offsetHeight;
					let order_footer = this.$refs.ti_order_footer.offsetHeight;
...
/*				let userId = window.localStorage.getItem('userId');
				...
				this.$axios.get2("/sc/tcOrder/ti/total", this.query)
					.then(function(response) {
						this.total = response.data.data;
						this.getSummaries();
					}.bind(this));*/
			...
			exportExcelSure() {
				this.$axios.post3('/sc/tcOrder/exportExcelListTe', this.query)
					.then(function(response) {
						...
						downloadElement.download = 'TI订单' + '.xls'; // 下载后文件名
			...
			init() {
				...
				if (buttonInfo.indexOf('ti_order_save') > -1) {
					this.permissionButtonForSave = true
				}
				if (buttonInfo.indexOf('ti_order_view') > -1) {
				...
				if (buttonPermission.some(item => item == 'ti_order_twoCode')) {
					this.permissionButtonForTwoCode = true
				}
				if (buttonInfo.indexOf('ti_order_checkTwoCode') > -1) {
					this.permissionButtonForViewTwoCode = true
				}
		/*		if (buttonInfo.indexOf('ti_order_shareTwoCode') > -1) {
					this.permissionButtonForShareTwoCode = true
				}*/

				if (buttonPermission.some(item => item == 'ti_order_print')) {
					this.permissionButtonForPrint = true
				}

				if (buttonInfo.indexOf('ti_order_print_billView') > -1) {
					this.permissionButtonForPrintBillView = true
				}
				if (buttonInfo.indexOf('ti_order_print_billSend') > -1) {
					this.permissionButtonForPrintBillSend = true
				}
				if (buttonPermission.some(item => item == 'ti_order_specialHandle')) {
					this.permissionButtonForSpecialHandle = true
				}
				if (buttonInfo.indexOf('ti_order_specialHandle_copy') > -1) {
					this.permissionButtonForSpecialHandleForceStop = true
				}
				if (buttonInfo.indexOf('ti_order_specialHandle_forceStop') > -1) {
					this.permissionButtonForSpecialHandleCopy = true
				}
				if (buttonInfo.indexOf('ti_order_export') > -1) {
		...
		created() {
			this.$axios.get('/hrs/org/getOrderFinanceLockView/TI').then(function(response) {
          ...
          .then(function(response) {
			...
			if (this.$route.query.flag) {
				if (this.$route.query.name == 'view') {
					...
					this.frow = JSON.parse(this.base64Decode(this.$route.query.frow))
					this.editVisible = true;
				} else if (this.$route.query.name == 'save' || this.$route.query.name == 'copy') {
					this.frow = JSON.parse(this.base64Decode(this.$route.query.frow))
					this.saveVisible = true;
