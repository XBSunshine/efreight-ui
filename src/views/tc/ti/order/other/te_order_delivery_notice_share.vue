<template>
  <div class="order-delivery-notice-share">
  <el-dialog title="二维码 - 分享" width="800px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" style="top:-80px;">
    <el-form :model="ruleForm" :rules="rules" label-width="100px" ref="ruleForm">
      ...
      <el-form-item label="网址/二维码" prop="website">
        <el-row >
          <el-col :span="19">
            <el-input type="textarea" disabled :rows="6" v-model="ruleForm.website" style="height: 151px"></el-input>
          </el-col>
...
/*        let userEmail = window.localStorage.getItem('userEmail')
      ...
      loadCcUserList(coopId){
        ...
        this.$axios.get2('/prm/coop/queryContactsIsValid', params)
          .then(function(response) {
            ...
            let contactsNameStr = this.frow.contactNameSe;
            let distDIC = {};
            if(contactsNameStr){
      ...
      initNormalData(){
        ...
        this.ruleForm.subject = "订单送货通知：" + (this.frow['rwbNumber'] || this.frow['orderCode']);
        this.ruleForm.content = this.frow.customerName+"，您好：\n\n" +
          "请点击正文的链接，或者手机扫描二维码，自助查看您的送货通知。" +
          "\n\n" +
          "谢谢。";

        this.ruleForm.website = this.frow['shareUrl'];
      ...
      qrCodeCallBack(data){
        let text = this.frow['rwbNumber'] || this.frow['orderCode'];
