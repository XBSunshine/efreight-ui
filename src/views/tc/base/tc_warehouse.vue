<template>
						...
						<el-form-item label-width="10px">
							<el-input style="width:185px;">
								...
								<el-select slot="suffix" v-model="query.businessScope" placeholder="请选择" style="width:119px;margin-right: -5px;" @change="businessScopeChange">
									<el-option v-for="item in businessScopeList" :key="item.paramText" :label="item.paramText" :value="item.paramText">
									</el-option>
								</el-select>
							</el-input>
						</el-form-item>
    ...
    <tc-warehouse-save ref="addMission" v-if="saveVisible" :visible.sync="saveVisible" :frow="frow"></tc-warehouse-save>
     <tc-warehouse-edit nce-edit ref="addMission" v-if="editVisible" :visible.sync="editVisible" :frow="frow"></tc-warehouse-edit>
	</div>
</template>
<script>
  import TcWarehouseSave from './warehouse/tc_warehouse_save.vue';
  import TcWarehouseEdit from './warehouse/tc_warehouse_edit.vue';

	export default {
	  components: {
	    'tc-warehouse-save': TcWarehouseSave,
      'tc-warehouse-edit': TcWarehouseEdit,
    },
		data() {
			return {
				loading: false,
		...
		created: function() {
      //业务范畴
      this.$axios.get('/sc/tcCategory/业务范畴').then(function(response) {
      ...
      operateName(row, column){
        ...
        this.frow.businessScope = this.query.businessScope;
			...
			queryList() {
			  ...
			  this.loading = true;
				this.$axios.get2("/sc/tc-warehouse/page?size=" + this.pageConf.pageSize + "&current=" + this.pageConf.pageCode, this.query).then((response)=> {
			...
			queryList2(current) {
				this.$axios.get2("/sc/tc-warehouse/page?size=" + this.pageConf.pageSize + "&current=" + current, this.query).then((response)=> {
					this.data = response.data.data.records;
					this.pageConf.totalPage = response.data.data.total;
				}).catch(function(error) {
					console.log(error);
      ...
      deleteW(row){
        this.$confirm('你确定要删除么？', '提示', {
           ...
           let param = {
            ...
            this.$axios.post2('/sc/tc-warehouse/delete', param)
