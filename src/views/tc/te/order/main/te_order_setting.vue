...
<script>
  import columns from './te_order_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_te_order_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.findByPage();
     ...
     sureSave(){
       ...
       if(this.multipleSelection&&this.multipleSelection.length>0){
         ...
         localStorage.removeItem(userId+'_te_order_list_column_width');//清除掉页面拖动字段宽度的缓存
         /*localStorage.setItem(userId+'_te_list_column',JSON.stringify(this.multipleSelection));
         this.handleClose();*/
       }else{
    ...
    sortBykey(ary, key) {
      ...
      this.tableColumns =  this.sortBykey(this.tableColumns, 'index');
      /*let  te_list_column = window.localStorage.getItem(userId+'_te_list_column');
      if(te_list_column){
          this.tableColumns = JSON.parse(info);
      }else{
        this.tableColumns = JSON.parse(info);
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  te_order_list_column_width = window.localStorage.getItem(userId+'_te_order_list_column_width');
      //从数据库查询设置信息
      let pageName = '订单操作管理/铁路出口/TE订单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.te_order_list_column = response.data.data;

              //如果te_order_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(te_order_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(te_order_list_column_width);
              ...
              if(this.te_order_list_column && this.te_order_list_column.length>0){
                  let loaclJsonArray = this.te_order_list_column;
      ...
      /*if(te_list_column){
        let loaclJsonArray = JSON.parse(te_list_column);
        if(loaclJsonArray&&loaclJsonArray.length>0){
          let liftNo = [];
          let lift = [];
           for(var i=0;i<this.tableColumns.length;i++){
