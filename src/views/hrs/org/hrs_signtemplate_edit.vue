<template>
	<el-dialog title="签约模板 - 编辑" width="50%" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
...
<script>
    ...
    inject: ['findByPage'],
      ...
      this.ruleForm.orgUserCount = this.frow.orgUserCount;
      if(this.frow.orgStatus == true){
          this.ruleForm.orgStatus = '1'
      }else{
          this.ruleForm.orgStatus = '0'
      }
			this.initData();
    },
		mounted: function() {
		...
		methods: {
			...
			submitPermission(formName) {
                ...
                //console.log("看一下参数"+permissionIds);
        this.$refs[formName].validate((valid) => {
            if (valid) {
                this.$axios.post2('/hrs/org/editSignTemplateAndPermission', {
