<template>
	<el-dialog title="系统权限管理 - 编辑" width="630px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
		<el-form :model="ruleForm" :rules="rules" ref="ruleForm" class="demo-ruleForm">
			...
			<el-row>
				<el-col class="elementWidth">
					<el-form-item label="帮助文档" label-width="85px">
						<el-upload class="" :action="actionPath" :accept="accept3" :data="uptoken" :before-upload="beforeAvatarUpload3" :on-remove="handleRemoveChange3" :on-success="handleSuccessChange3" ref="upload3" :auto-upload="true" :show-file-list="false">
							<el-input v-model="ruleForm.helpDocumentUrl" auto-complete="off" disabled style="width: 360px;">
							</el-input>
							<el-button size="mini" type="primary">上传</el-button>&nbsp;&nbsp;&nbsp;&nbsp;
							<el-button slot="tip" size="mini" type="danger" @click="removtePdf">删除</el-button>
						</el-upload>
					</el-form-item>
					<!--<el-form-item label="" label-width="85px">
            <el-upload    v-if="ruleForm.helpDocumentUrl==null||ruleForm.helpDocumentUrl==''"
                          :action="actionPath"
                          :accept="accept3"
                          :limit=1
                          :data="uptoken"
            ...
            <ul class="el-upload-list el-upload-list&#45;&#45;text" v-if="ruleForm.helpDocumentUrl!=null &&ruleForm.helpDocumentUrl!=''">
              <li class="el-upload-list__item is-success">
                ...
                <el-link :href="ruleForm.helpDocumentUrl" target="_blank">{{ruleForm.helpDocumentName}}</el-link>
...
<script>
 ...
 /*     beforeAvatarUpload3(file) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$axios.put('/hrs/permission', params)
              .then(function(response) {
    ...
    created(){
      this.$axios.get('/hrs/permission/'+this.frow.id)
      .then(function(response) {
        if(response.data.code==0){
          this.ruleForm.permissionId=response.data.data.permissionId ;
          this.ruleForm.permissionCode=response.data.data.permissionCode ;
          this.ruleForm.permissionName=response.data.data.permissionName ;
          this.ruleForm.path=response.data.data.path ;
          this.ruleForm.url=response.data.data.url ;
          this.ruleForm.permission= response.data.data.permission ;
          this.ruleForm.icon= response.data.data.icon ;
          this.ruleForm.parentId= response.data.data.parentId ;
          this.ruleForm.parentIds= response.data.data.parentIds ;
          this.ruleForm.sort=response.data.data.sort ;
          this.ruleForm.status= response.data.data.status ;
          this.ruleForm.permissionType= response.data.data.permissionType ;
          this.ruleForm.appCode= response.data.data.appCode ;
          this.ruleForm.disabled= response.data.data.disabled ;
          this.ruleForm.adminDefault= response.data.data.adminDefault ;
          this.ruleForm.helpDocumentUrl= response.data.data.helpDocumentUrl ;
          this.ruleForm.helpDocumentName= response.data.data.helpDocumentName ;
        }else{
          this.openError(response.data.messageInfo);
        }
       }.bind(this)
