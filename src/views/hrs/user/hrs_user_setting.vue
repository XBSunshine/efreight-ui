...
<script>
  import columns from './hrs_user_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_hrs_user_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.findByPage();
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  hrs_user_list_column_width = window.localStorage.getItem(userId+'_hrs_user_list_column_width');
      //从数据库查询设置信息
      let pageName = '组织机构管理/用户管理';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.hrs_user_list_column = response.data.data;

              //如果hrs_user_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(hrs_user_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(hrs_user_list_column_width);
              ...
              if(this.hrs_user_list_column && this.hrs_user_list_column.length>0){
                  let loaclJsonArray = this.hrs_user_list_column;
