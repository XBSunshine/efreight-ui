<template>
	<el-dialog title="部门管理 - 合并" width="85%" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
		<div>
			<div>
				<el-table :data="data1" row-key="deptId" border default-expand-all :tree-props="{children: 'children', hasChildren: 'hasChildren'}" height="520px">
					<el-table-column type="index" label="操作" width="60" align="center">
						<template slot-scope="scope">
							<el-button type="text" size="small" v-on:click="doSelect(scope.row)" v-show="!scope.row.children.length>0 && scope.row.deptCode !=frow.deptCode">合并</el-button>
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			doSelect(row) {
				...
				let params = '{"deptCode":' + row.deptCode + ',"budgetHc":' + row.budgetHc + ',"isFinalProfitunit":' + row.isFinalProfitunit + ',"deptName":"' + row.deptName + '","deptCodeSelect":' + this.frow.deptCode + '}';
				this.$axios.post2('/hrs/dept/mergeById', params)
					.then(function(response) {
