<template>
  <el-dialog title="设置字段" width="580px" :visible.sync="visible" append-to-body :close-on-press-escape="false"
             :close-on-click-modal="false" :before-close="handleClose" style="top:-80px;">
    ...
    <div class="af_order_class">
      <el-table :data="tableColumns" height="480" tooltip-effect="dark" ref="multipleTable"
        ...
        <el-table-column align="center" prop="index" width="80" label="显示顺序">
          <template slot-scope="scope">
            <el-input maxlength="3" v-if="scope.row.prop=='constitute'" disabled size="small" v-model="scope.row.index"
                      @input="checkNumber(scope.row,'index')">
            </el-input>
            <el-input maxlength="3" v-else size="small" v-model="scope.row.index"
                      @input="checkNumber(scope.row,'index')">
            </el-input>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="width" width="80" label="显示宽度">
          <template slot-scope="scope">
            <el-input maxlength="3" v-if="scope.row.prop=='constitute'" disabled size="small" v-model="scope.row.width"
                      @input="checkNumber(scope.row,'width')">
            </el-input>
            <el-input maxlength="3" v-else size="small" v-model="scope.row.width"
                      @input="checkNumber(scope.row,'width')">
            </el-input>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="sortable" width="120" label="允许排序">
          <template slot-scope="scope">
            <el-checkbox v-if="scope.row.prop=='constitute'" disabled name="sort"
                         v-model="scope.row.sortable"></el-checkbox>
...
<script>
import columns from './hrs_doc_column.json'
...
export default {
  ...
  methods: {
    ...
    handleClose() {
      ...
      localStorage.removeItem(userId + '_hrs_doc_column_width'); //清除掉页面拖动字段宽度的缓存
      this.findByPage();
    ...
    setLabel(tableColumns) {
      let indexDelete = [],
        ...
        hiddenDic = this.frow.hiddenDic[businessScope];

      tableColumns.forEach((column, index) => {
        let columnName = column.prop;
        let labelName = this.frow.labelName[columnName];
        if (labelName) {
          column.label = labelName[businessScope] || labelName.OTHER;
        }
        if (hiddenDic && hiddenDic[columnName]) {
  ...
  created() {
    let afInfo = JSON.stringify(columns.info);
  ...
  mounted() {
    this.setLabel(this.tableColumns);
    let userId = window.localStorage.getItem('userId');
    let af_list_column_width = window.localStorage.getItem(userId + '_hrs_doc_column_width');
    //从数据库查询设置信息
    this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + this.pageName)
      .then(function (response) {
        this.af_list_column = response.data.data;
