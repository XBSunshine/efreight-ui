...
<script>
	import columns from './css_wirteoff_column.json'
	...
	export default {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_css_payment_invoice_writeoff_list_column_width'); //清除掉页面拖动字段宽度的缓存
			...
			sureSave() {
				...
				if (this.multipleSelection && this.multipleSelection.length > 0) {
					...
					this.$axios.post2('/hrs/user/saveUserPageSet', {
							...
							pageName: '财务结算管理/成本对账/核销单'
						})
						.then(function(response) {
							if (response.data.code == 0) {
								this.colomnSetCallback(this.multipleSelection)
								this.handleClose()
							} else {
		...
		created() {
			let cssWriteoffInfo = JSON.stringify(columns.cssWriteoffInfo);
			this.oldTableColumns = cssWriteoffInfo;
			this.tableColumns = JSON.parse(cssWriteoffInfo);
			this.tableColumns = this.sortBykey(this.tableColumns, 'index');
		},
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let af_writeoff_list_column_width = window.localStorage.getItem(userId + '_css_payment_invoice_writeoff_list_column_width');
			//从数据库查询设置信息
			...
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
					...
					//如果af_writeoff_list_column_width不为空，则说明在页面拖动过字段的宽度
					if (af_writeoff_list_column_width) {
						let loaclWidthJsonArray = JSON.parse(af_writeoff_list_column_width);
							...
							for (var i = 0; i < this.tableColumns.length; i++) {
								...
								for (var j = 0; j < loaclJsonArray.length; j++) {
									if (this.tableColumns[i].prop == loaclJsonArray[j].prop) {
										...
										if (af_writeoff_list_column_width) {} else {
