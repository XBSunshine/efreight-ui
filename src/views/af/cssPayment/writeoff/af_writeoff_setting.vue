...
<script>
  import columns from './af_wirteoff_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_af_writeoff_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.queryList();
    ...
    sortBykey(ary, key) {
      return ary.sort(function (a, b) {
		...
		created() {
      let afWriteoffInfo = JSON.stringify(columns.afWriteoffInfo);
      this.oldTableColumns =afWriteoffInfo;
      //从数据库查询设置信息
      ...
      if(this.list_column){
          this.tableColumns = JSON.parse(afWriteoffInfo);
      }else{
          this.tableColumns = JSON.parse(afWriteoffInfo);
          this.tableColumns =  this.sortBykey(this.tableColumns, 'index');
      }
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  af_writeoff_list_column_width = window.localStorage.getItem(userId+'_af_writeoff_list_column_width');
      //从数据库查询设置信息
      let pageName = '财务结算管理/成本对账/核销单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
