<template>
	<el-dialog title="对账单 - 核销" width="1100px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" style="top:-100px;" append-to-body>
		<el-form :model="ruleForm" ref="ruleForm" label-width="20px" class="demo-ruleForm writeoffMinusSavePage">
			<el-row>
			...
			<el-row>
				<el-col class="elementWidth">
					<el-form-item>
						<el-input v-if="ruleForm.currency=='CNY'" v-model="ruleForm.currency" style="width:200px;" disabled>
							<template slot="prepend">
								<span>核销金额</span>
							</template>
						</el-input>
						<el-input v-else v-model="ruleForm.currency" class="currencyRedColor" style="width:200px;" disabled>
							<template slot="prepend">
								<span>核销金额</span>
							</template>
						</el-input>
				...
				<el-col class="elementWidth">
					<el-form-item label-width="15px">
						<el-input v-model="ruleForm.amountWriteoff" disabled style="width:200px;">
						</el-input>
					</el-form-item>
				</el-col>
        <el-col class="elementWidth">
          <el-form-item>
            <el-input style="width:300px;margin-left: -10px;">
              ...
              <el-select slot="suffix" v-model="ruleForm.financialAccountName" filterable placeholder="请选择" style="width:251px;margin-right: -5px;" @change="setFinancialAccount">
				...
				<!--<el-col class="elementWidth">
					<el-form-item label-width="35px">
						<el-input auto-complete="off" disabled style="width:275px" v-model="ruleForm.writeoffNum">
							<template slot="prepend">
								<span>核销编号</span>
							</template>
						</el-input>
		...
		<el-table ref="table" v-loading="loading" :data="cssCostWriteoffDetails" border stripe @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" label="本次核销金额" width="150">
				<template slot-scope="scope">
					<el-input :disabled="scope.row.amountPaymentNoWriteoff==0" @input="formatFloat2ForTableField(scope.row)" @change="changeCostCurrAmountPayment(scope.row)" v-model="scope.row.amountWriteoff" style="width:90px;" disabled></el-input>
...
<script>
	export default {
		...
		created: function() {
			...
			this.ruleForm.amountWriteoff = this.frow.amountPaymentNoWriteoff
      ...
      setFinancialAccount(vId) {
        ...
        if (!this.ruleForm.financialAccountName) {
            ...
            return;
				...
				if(this.cssCostWriteoffDetails.length != this.ruleForm.cssCostWriteoffDetails.length) {
					this.openError("该对账单未核销金额小于0，必须全部核销")
					return
				}
