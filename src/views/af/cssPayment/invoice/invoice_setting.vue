...
<script>
	import columns from './invoice_column.json'
	...
	export default {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_css_invoice_list_column_width'); //清除掉页面拖动字段宽度的缓存
		...
		created() {
			let invoiceInfo = JSON.stringify(columns.invoiceInfo);
			this.oldTableColumns = invoiceInfo;
			this.tableColumns = JSON.parse(invoiceInfo);
			this.tableColumns = this.sortBykey(this.tableColumns, 'index');
		},
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let af_payment_list_column_width = window.localStorage.getItem(userId + '_css_invoice_list_column_width');
			//从数据库查询设置信息
			let pageName = '财务结算管理/成本对账/发票';
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
					this.list_column = response.data.data;

					//如果af_payment_list_column_width不为空，则说明在页面拖动过字段的宽度
					if (af_payment_list_column_width) {
						let loaclWidthJsonArray = JSON.parse(af_payment_list_column_width);
							...
							for (var i = 0; i < this.tableColumns.length; i++) {
								...
								for (var j = 0; j < loaclJsonArray.length; j++) {
									if (this.tableColumns[i].prop == loaclJsonArray[j].prop) {
										...
										if (af_payment_list_column_width) {} else {
