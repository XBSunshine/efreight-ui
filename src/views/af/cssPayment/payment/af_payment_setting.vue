...
<script>
  import columns from './af_payment_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_af_payment_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.queryList();
    ...
    sortBykey(ary, key) {
      return ary.sort(function (a, b) {
		...
		created() {
      //let  userId = window.localStorage.getItem('userId');
      let afPaymentInfo = JSON.stringify(columns.afPaymentInfo);
      this.oldTableColumns =afPaymentInfo;
      ...
      if(this.list_column){
          this.tableColumns = JSON.parse(afPaymentInfo);
      }else{
          this.tableColumns = JSON.parse(afPaymentInfo);
          this.tableColumns =  this.sortBykey(this.tableColumns, 'index');
      }
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  af_payment_list_column_width = window.localStorage.getItem(userId+'_af_payment_list_column_width');
      //从数据库查询设置信息
      let pageName = '财务结算管理/成本对账/对账单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
