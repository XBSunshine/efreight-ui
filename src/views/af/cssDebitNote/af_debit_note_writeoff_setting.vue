   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_af_debit_note_writeoff_column_width');//清除掉页面拖动字段宽度的缓存
      	this.queryList();
     ...
     sureSave(){
       ...
       if(this.multipleSelection&&this.multipleSelection.length>0){
         ...
         localStorage.removeItem(userId+'_af_debit_note_writeoff_column_width');//清除掉页面拖动字段宽度的缓存
   /*      this.multipleSelection = this.sortBykey(this.multipleSelection, 'index');
         localStorage.setItem(userId+'_af_debit_note_writeoff_column',JSON.stringify(this.multipleSelection));
         this.handleClose();*/
       }else{
	...
	created() {
      ...
      //let  af_debit_note_writeoff_column = window.localStorage.getItem(userId+'_af_debit_note_writeoff_column');
      //从数据库查询设置信息
      let pageName = '财务结算管理/收入对账/核销单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.af_debit_note_writeoff_column = response.data.data;
          }.bind(this));

      if(this.af_debit_note_writeoff_column){
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  af_debit_note_writeoff_column_width = window.localStorage.getItem(userId+'_af_debit_note_writeoff_column_width');
      //从数据库查询设置信息
      let pageName = '财务结算管理/收入对账/核销单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              ...
              //如果af_debit_note_writeoff_column_width不为空，则说明在页面拖动过字段的宽度
              if(af_debit_note_writeoff_column_width){
                  let loaclWidthJsonArray = JSON.parse(af_debit_note_writeoff_column_width);
              ...
              if(this.af_debit_note_writeoff_column && this.af_debit_note_writeoff_column.length>0){
                  let loaclJsonArray = this.af_debit_note_writeoff_column;
...
/*      if(af_debit_note_writeoff_column){
        let loaclJsonArray = JSON.parse(af_debit_note_writeoff_column);
        if(loaclJsonArray&&loaclJsonArray.length>0){
          let liftNo = [];
          let lift = [];
           for(var i=0;i<this.tableColumns.length;i++){
