<template>
  <el-dialog title="账单 - 核销" width="1000px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" style="top:-50px;" >
...
<!--
      ...
      <!-- <el-row>
        <el-col class="elementWidth">
          <el-form-item label="" >
            <el-input class="showWordLimit widthWithTwo" v-model="ruleForm2.writeoffRemark" auto-complete="off" style="width:960px" show-word-limit maxlength="200" clearable>
    ...
    <div style="margin:10px 10px 10px 10px;">
      ...
      <el-table :data="data1" border >
        ...
        <el-table-column align="center" prop="exchangeRate" label="汇率" width="60"></el-table-column>
        <el-table-column align="center" label="本次核销金额" width="160">
          <template slot-scope="scope">
          <el-input class="orderEditPage222" v-model="scope.row.amountWriteoff2" maxlength="11" @input="formatToFloat2(scope.row,'amountWriteoff2')" :disabled="scope.row.currency!=ruleForm2.currency || flag3" @change="change11(scope.row)" style="width:115px">
          </el-input>
        ...
        <el-table-column align="right" label="本次核销金额（本币）" width="160">
          <template slot-scope="scope">
          <el-input class="orderEditPage222" v-model="scope.row.functionalAmountWriteoff2" maxlength="11" @input="formatToFloat2(scope.row,'functionalAmountWriteoff2')" disabled @change="change22(scope.row)">
          </el-input>
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    created: function(){
      ...
      this.ruleForm2.debitNoteNum=this.frow.debitNoteNum;
      this.$axios.post('/afbase/cssIncomeWriteoff/queryBillCurrency/'+this.frow.debitNoteId)
      .then(function(response) {
        ...
        if (isHaved==1 && this.frow.functionalAmount<0) {
          ...
          this.data1[k].amountWriteoff2=parseFloat(this.data1[k].amountWriteoffNo).toFixed(2);
            this.data1[k].functionalAmountWriteoff2=parseFloat(this.data1[k].functionalAmountWriteoffNo).toFixed(2);
        }
        // 格式
        for (var i = 0; i < this.data1.length; i++) {
    ...
    methods: {
      ...
      changeRowCheck(row) {
        if (row.checkBox) {
          // if (row.currency==this.ruleForm2.currency || this.ruleForm2.currency=='CNY') {
      ...
      change00(){
        ...
        for (var i = 0; i < this.data1.length; i++) {
          if (this.data1[i].currency==this.ruleForm2.currency) {
                ...
                this.data1[i].amountWriteoff2=parseFloat(this.ruleForm2.amountWriteoff).toFixed(2);
                this.data1[i].functionalAmountWriteoff2=parseFloat(this.data1[i].amountWriteoff2*this.data1[i].exchangeRate).toFixed(2);
      ...
      doSave(){
        ...
        this.ruleForm2.debitCurrencyList=this.data1;
        this.$axios.post2('/afbase/cssIncomeWriteoff/doBillWriteoff', this.ruleForm2)
          .then(function(response) {
      ...
      doSave2(){
        ...
        for (var i = 0; i < this.data1.length; i++) {
          ...
          if (this.ruleForm2.currency!='CNY' && this.ruleForm2.currency==this.data1[i].currency && parseFloat(this.ruleForm2.amountWriteoff) !=parseFloat(this.data1[i].amountWriteoff2)) {
            this.openError("本次核销金额与核销金额不等");
            return;
          }
        }
        // alert(this.functionalAmountNo);
        // alert(this.total2);
        if(Math.abs(this.functionalAmountNo)<Math.abs(this.total2)) {
          this.openError("核销金额不能大于未核销金额");
          return;
        }
        if(Math.abs(this.ruleForm2.amountWriteoff)<=0) {
          this.openError("核销金额不能等于0");
