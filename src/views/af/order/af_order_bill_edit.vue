				...
				<el-row>
					<el-col class="elementWidth">
						<el-form-item label="账单编号">
							<el-input auto-complete="off" disabled v-model="ruleForm2.debitNoteNum" style="width:155px">
							</el-input>
						</el-form-item>
					</el-col>
					<el-col class="elementWidth">
	...
	export default {
		...
		created: function() {
			...
			this.$axios.get('/afbase/coopProject/selectCurrency').then(function(response) {
				...
				//查询
				let params = {
					debitNoteId: this.frow.dataBean.debitNoteId,
				...
				this.$axios.post2('/afbase/cssDebitNote/queryHavedBillDetail', params)
					.then(function(response2) {
						if (response2.data.code == 0) {
							this.data1 = this.data1.concat(response2.data.data);
							this.init();
						}
					}.bind(this));
			...
			var billParams = {
				debitId: this.frow.dataBean.debitNoteId,
				selections: selections
			}
			this.$axios.post2('/afbase/billcurrency/queryBill2', billParams)
				.then(function(response) {
		...
		methods: {
			init() {
				...
				this.setExchangeRate(this.frow.selections[0].incomeCurrency);
			...
			doSave() {
				...
				this.$axios.post2('/afbase/cssDebitNote/doUpdate', this.ruleForm2)
					.then(function(response) {
						if (response.data.code == 0) {
							this.openSuccess();
							this.callHandleClose(this.ruleForm2.debitNoteId);
							this.handleClose();
						} else {
