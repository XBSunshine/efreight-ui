<template>
	<el-dialog :title="'空运出口订单 - '+titleName " :fullscreen="ifFullscreen" width="1300px" :visible.sync="visible" append-to-body :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" :show-close="!ifFullscreen" :style="style">
...
<script>
	const editTab = resolve => require(['@/views/af/order/af_order_edit_tab.vue'], resolve)
	...
	export default {
      ...
      this.$axios.get('/afbase/afoporder/selectOrderStatus?node_name=财务锁账&order_uuid=' + this.frow.orderUuid).then(function(response) {
          this.frow.finishFlag=!response.data.data
			...
			if (buttonInfo.indexOf('af-order-edit') > -1||buttonInfo.indexOf('af-order-view') > -1) {
