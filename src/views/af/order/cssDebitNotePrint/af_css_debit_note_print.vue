			...
			<el-row>
				<el-col class="elementWidth">
					<el-form-item label="模板类型">
						<el-select v-model="modelType" placeholder="请选择" required style="width: 150px;">
	...
	export default {
		...
		methods: {
			printSubmit() {
				...
				if(this.frow.data[0].businessScope=='AE'){
					this.$axios.post('/afbase/debitNote/printManynew/' + this.modelType + '/' + debitNoteIds+'/AE').then((response) => {
						this.handleClose()
					 ...
					 this.$axios.post('/afbase/debitNote/printManynew/' + this.modelType + '/' + debitNoteIds+'/' +this.frow.data[0].businessScope).then((response) => {
						this.handleClose()
						window.open(response.data.data)
					}).catch(function(error) {
						console.log(error);
			...
			exportExcel(){
				...
				this.$axios.post3('/afbase/debitNote/exportExcel/'+ this.modelType + '/' + debitNoteIds+'/' +this.frow.data[0].businessScope)
