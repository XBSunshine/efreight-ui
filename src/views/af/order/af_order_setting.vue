...
<script>
  import columns from './af_order_column.json'
   ...
   provide() {
     ...
     sureSave(){
       ...
       if(this.multipleSelection&&this.multipleSelection.length>0){
         ...
         for(let i=0;i<this.multipleSelection.length;i++){
             ...
             // if(this.multipleSelection[i].sortable){
             //     sortColumn.push(this.multipleSelection[i]);
             // }
         }
         // if(sortColumn&&sortColumn.length>5){
         //    this.openError("结果排序最多勾选5个");
         //    return false;
         // }
         this.multipleSelection = this.sortBykey(this.multipleSelection, 'index');
         this.$axios.post2('/hrs/user/saveUserPageSet', {
             multipleSelection: this.multipleSelection,
             pageName: '订单操作管理/空运出口/AE订单'
             })
    ...
    sortBykey(ary, key) {
      return ary.sort(function (a, b) {
		...
		created() {
      //let  userId = window.localStorage.getItem('userId');
      let afInfo = JSON.stringify(columns.afInfo);
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  af_list_column_width = window.localStorage.getItem(userId+'_af_list_column_width');
      //从数据库查询设置信息
      let pageName = '订单操作管理/空运出口/AE订单';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
