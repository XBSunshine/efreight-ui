		...
		<div class="orderEditPage" style="width: 1280px;">
			<el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
				<div style="margin-top: -15px;margin-bottom: 20px;">
					<span style="font-size: large;margin-bottom: 20px;">空运出口订单 - 新增</span>
          ...
          <el-col class="elementWidth">
            <el-form-item>
              <el-input auto-complete="off" style="width:295px;margin-right: 15px;" clearable>
                ...
                <el-select slot="suffix" clearable  @input="ruleForm.departureStation = ruleForm.departureStation.toUpperCase()" remote :filter-method="searchAirport" filterable  :loading="loading" v-model="ruleForm.departureStation"
                ...
                <el-select slot="suffix"  @input="ruleForm.arrivalStation = ruleForm.arrivalStation.toUpperCase()"  remote :filter-method="searchAirport" filterable  :loading="loading" v-model="ruleForm.arrivalStation"
      ...
      changeClass(index, item){
        if(index == 0){
          ...
          this.airportList.forEach((item)=>{
            if(item.apCode == this.ruleForm.departureStation){
              contains = true;
            }
          })
          if(!contains){
            this.$nextTick(()=>{
              let apCode = item.apCode;
              let row = document.getElementById(apCode);
              if(row.parentNode.className.indexOf("selected") < 0){
...
<style>
  ...
  .widthWithThermometer .el-input__inner {
    line-height: 29px !important;
  }
