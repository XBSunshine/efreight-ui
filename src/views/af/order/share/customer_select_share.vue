<template>
	<el-dialog title="协作公司 - 选择" width="800px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" append-to-body>
	...
	export default {
		...
		methods: {
          ...
          this.handleClose();
			...
			queryList() {
        let param = {
          coopOrgId:this.frow.orderOrgId,
          coopName:this.query.coopName,
          isShare:1
        }
        this.$axios.get2('/prm/coopShare/page?size=' + this.pageConf.pageSize + '&current=' + this.pageConf.pageCode, param).then((response) => {
       ...
       let param = {
         ...
         isShare:1
				 ...
				 this.$axios.get2('/prm/coopShare/page?size=' + this.pageConf.pageSize + '&current=' + current, param).then((response) => {
					this.data1 = response.data.data.records;
					this.pageConf.totalPage = response.data.data.total;
				}).catch((error)=> {
					console.log(error);
