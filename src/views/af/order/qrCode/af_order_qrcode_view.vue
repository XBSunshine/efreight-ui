<template>
	<el-dialog title="订单跟踪二维码" :visible.sync="visible" width="400px" :destroy-on-close="true" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
	...
	export default {
		...
		created() {
			...
			if (this.frow.businessScope.startsWith('A')) {
				code = this.frow.awbNumber || this.frow.orderCode
			} else if (this.frow.businessScope.startsWith('S')) {
				code = this.frow.mblNumber || this.frow.orderCode
			}
			let url = location.origin + "/#/externalOrderTrack?o=" + scope + (this.frow['orderUuid'] || "") + "&code=" + code
