      ...
      <el-row>
        <el-col class="elementWidth">
          <el-form-item label="附件类型" prop="fileType" required label-width="115px">
            <el-select v-model="ruleForm.fileType" placeholder="请选择" style="width: 115px;" @change="checkFileType" :disabled="disabledFlag">
        ...
        <el-col class="elementWidth">
          <el-form-item label="是否对外显示" prop="fileType" required label-width="115px">
			...
			<el-row>
				<el-col class="elementWidth">
					...
					<el-form-item style="margin-left: 35px;">
						<!--<el-upload :action="actionPath" :data="uptoken" :on-change="uploadChange" :on-error="handleErrorChange" :on-success="handleSuccessChange" :file-list="fileList" ref="upload" :auto-upload="false">
            ...
            <el-upload      class="el-form-item__content3"
                            ...
                            :file-list="fileList"
                            :auto-upload="true">
              <el-button size="small" type="primary" @click="clickSelect" :disabled="disabledFlag">选择</el-button>&nbsp;
              <font size="1px">支持格式：PDF/DOC/DOCX/XLS/XLSX/TXT/RAR/ZIP/JPG/JPEG/BMP/PNG , 最大10M。</font>
            </el-upload>
					</el-form-item>
				</el-col>
			...
			<!--<el-row>
				<el-col class="elementWidth">
          <el-form-item style="margin-left: 35px;">
					  <span style="font-size: smaller;">支持格式：PDF/DOC/DOCX/XLS/XLSX/TXT/RAR/ZIP/JPG/JPEG/BMP/PNG , 最大10M。</span>
          </el-form-item>
				</el-col>
			</el-row>-->
			<!--<el-row>
			...
			<el-row>
				<el-form-item style="margin-left: 15px;">
					<el-button :loading="loading" :disabled="uptoken.token==null||uptoken.token==''" size="mini" type="primary" @click="submitForm('ruleForm')">保存</el-button>
				</el-form-item>
			</el-row>
		</el-form>
	</el-dialog>
	...
	export default {
      ...
      beforeAvatarUpload3(file,fileList) {
          ...
          if(this.ruleForm.fileLists.length > 0){
              for(let j = 0; j < this.ruleForm.fileLists.length; j++) {
                  if(this.ruleForm.fileLists[j].name === file.name.substring(0, file.name.lastIndexOf('.'))) {
                      this.isRepeat = 'repeat'
                  }
              }
          }
          if(this.isRepeat === 'repeat'){
              this.openError("附件名称不能重复")
              return false;
          }else{*/
      ...
      handleRemoveChange3(file, fileList){
          this.ruleForm.fileLists.forEach(function(item, index, arr) {
              if(item.name == file.name.substring(0, file.name.lastIndexOf('.'))) {
                  arr.splice(index, 1);
              }
          });
      },
      handleSuccessChange3(response, file, fileList) { //上传成功后在图片框显示图片
          ...
          this.ruleForm.fileLists.push(item);*/
         // this.ruleForm.fileLists = fileList
        ...
        this.noExecute = true
        this.$axios.post2('/afbase/orderFiles/doBatchSave', this.ruleForm)
            .then(function(response) {
      ...
      buildUploadFileKey(file) {
        ...
        return 'org/' + new Date().format("yyMM") + '/' + orgUuid + "/order_attachment_" + new Date().format("ddhhmmss") + "_" + new Date().getTime() + file.name.substring(file.name.lastIndexOf('.'));
      }
    }
	}
</script>
