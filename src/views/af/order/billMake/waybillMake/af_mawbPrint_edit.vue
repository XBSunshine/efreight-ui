<template>
	<!--<el-dialog title="单证制作 - 编辑主单" top="1vh" width="1100px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">-->
	<div>
		<form>
				...
				<div id="form-main">
					<div id="form-title" class="horizontal">
						...
						<div style="margin-left: 320px;display: inline;">
							<!-- <el-button :loading="loading" type="primary" size="small" @click="newSelect" v-if="editMawbFlag" :disabled="ifDisabled">新建/查询</el-button> -->
							<el-button :loading="loading" style="margin-left: 5px;" type="primary" size="small" @click="modify" v-if="editMawbFlag" :disabled="ifDisabled">修改</el-button>
				            <el-button style="margin-left: 5px;" type="primary" size="small" @click="printG" :disabled="ifDisabled">打印运单(格打)</el-button>
				            <el-button style="margin-left: 5px;" type="primary" size="small" @click="printT" :disabled="ifDisabled">打印运单(套打)</el-button>
				            <el-button style="margin-left: 5px;" :loading="loading" type="primary" size="small" @click="checkSend('AMS')" v-if="sendMawbFlag" :disabled="ifDisabled">发送运单</el-button>
				            <el-button style="margin-left: 5px;" :loading="loading" type="primary" size="small" @click="checkSend('TB')" v-if="sendMawbFlag" :disabled="ifDisabled">发送货站</el-button>
						...
						<div id="form-left">
							...
							<div id="routing">
								...
								<div id="second-to">
									...
									<input v-model="ruleForm.transitStation" type="text" id="mawb_to2" name="mawb_to2" class="TextBox" maxlength="4" @change="changeArrivalStationName" @input="ruleForm.transitStation=strTrim(ruleForm.transitStation).toUpperCase()">
								</div>
								<div id="second-carrier">
									<span class="label">by</span>
									<input v-model="ruleForm.transitStationBy" type="text" id="mawb_by2" name="mawb_by2" class="TextBox" maxlength="2" @change="ruleForm.transitStationBy=strTrim(ruleForm.transitStationBy).toUpperCase()">
								</div>
								<div id="third-to">
									<span class="label">to</span>
									<input v-model="ruleForm.transitStation2" type="text" id="mawb_to3" name="mawb_to3" class="TextBox" maxlength="4" @change="changeArrivalStationName" @input="ruleForm.transitStation2=strTrim(ruleForm.transitStation2).toUpperCase()">
								</div>
								<div id="third-carrier">
									<span class="label">by</span>
									<input v-model="ruleForm.transitStationBy2" type="text" id="mawb_by3" name="mawb_by3" class="TextBox" maxlength="2" @change="ruleForm.transitStationBy2=strTrim(ruleForm.transitStationBy2).toUpperCase()">
								</div>
							</div>
							<div id="flight">
						...
						<div id="form-right">
							...
							<div id="accounting">
								<div id="information">
									...
									<textarea maxlength="400" v-model="ruleForm.accountingInformation" id="Accounting" name="Accounting" rows="4" cols="61" @change="ruleForm.accountingInformation=strTrim(ruleForm.accountingInformation).toUpperCase()"></textarea>
								</div>
								<div id="reference">
                    ...
                    <a class="label" href="javascript:void(0)" @click="getTact" style="color: #137DFA;text-decoration: underline;">Rate/Charge</a>
								...
								<div id="item-body">
									<div id="item-record">
										...
										<div class="space horizontal"></div>
										<div id="chargeable-weight" class="horizontal">
											<input maxlength="10" @change="computeFreightCombined" @input="checkFloat('awbChargeWeight')" v-model="ruleForm.awbChargeWeight" id="chargeableweight" name="chargeableweight" class="TextBox">
										</div>
										<div class="space horizontal"></div>
										<div id="rate-charge" class="horizontal">
											<input maxlength="10" @change="computeFreightCombined" @input="checkFloat('rateCharge')" v-model="ruleForm.rateCharge" id="rateCharge" name="rateCharge" class="TextBox">
										</div>
										<div class="space horizontal"></div>
										<div id="total" class="horizontal">
										...
										<div class="space horizontal"></div>
										<div id="goods" class="horizontal">
											<textarea maxlength="1500" v-model="ruleForm.goodsDescription" id="GoodsDescription" name="GoodsDescription" rows="4" cols="61" @change="ruleForm.goodsDescription=strTrim(ruleForm.goodsDescription).toUpperCase()"></textarea>
										</div>
									</div>
									<div id="item-record">
										<div id="pieces" class="horizontal">
							...
							<div id="summary">
								<div id="left" class="horizontal">
									...
									<div id="valuation-charge">
										...
										<div id="prepaid-input">
											<input maxlength="10" @change="computePrepaidTotal" @input="checkFloat('prepaidValuationCharge')" :disabled="ruleForm.payMethod=='CC'" v-model="ruleForm.prepaidValuationCharge" id="prepaid_valuation_charge" name="prepaid_valuation_charge" class="TextBox">
										</div>
										<div id="collect-input">
											<input maxlength="10" @change="computeCollectTotal" @input="checkFloat('collectValuationCharge')" :disabled="ruleForm.payMethod=='PP'" v-model="ruleForm.collectValuationCharge" id="collect_valuation_charge" name="collect_valuation_charge" class="TextBox">
										</div>
									</div>
									<div id="tax">
										<span class="label horizontal">Tax</span>
										<div id="prepaid-input">
											<input maxlength="10" @change="computePrepaidTotal" @input="checkFloat('prepaidTax')" :disabled="ruleForm.payMethod=='CC'" v-model="ruleForm.prepaidTax" id="prepaid_tax" name="prepaid_tax" class="TextBox">
										</div>
										<div id="collect-input">
											<input maxlength="10" @change="computeCollectTotal" @input="checkFloat('collectTax')" :disabled="ruleForm.payMethod=='PP'" v-model="ruleForm.collectTax" id="collect_tax" name="collect_tax" class="TextBox">
										</div>
									</div>
									...
									<div id="total-other-charges-due-agent">
										...
										<div id="prepaid-input">
											<input @change="computePrepaidTotal" maxlength="10" @input="checkFloat('prepaidTotalOtherChargeDueAgent')" :disabled="ruleForm.payMethodOther=='CC'" v-model="ruleForm.prepaidTotalOtherChargeDueAgent" name="mawb_agentotherprepaid" id="mawb_agentotherprepaid" class="TextBox">
										</div>
										<div id="collect-input">
											<input @change="computeCollectTotal" maxlength="10" @input="checkFloat('collectTotalOtherChargeDueAgent')" :disabled="ruleForm.payMethodOther=='PP'" v-model="ruleForm.collectTotalOtherChargeDueAgent" name="mawb_agentothercollect" id="mawb_agentothercollect" class="TextBox">
										</div>
									</div>
								...
								<div id="right" class="horizontal">
									...
									<div id="signature-of-agent">
										...
										<div id="signature-input">
											<input maxlength="200" v-model="ruleForm.shipperSignature" name="input_signature_of_shipper_or_his_agent" type="text" id="input_signature_of_shipper_or_his_agent" class="TextBox" @change="ruleForm.shipperSignature=strTrim(ruleForm.shipperSignature).toUpperCase()">
										...
										<div id="signature-input-statement">
											<input maxlength="300" v-model="ruleForm.signatureofShipperorhisAgent" name="signature_of_shipper_or_his_agent_statement" type="text" id="signature_of_shipper_or_his_agent_statement" class="TextBox" @change="ruleForm.signatureofShipperorhisAgent=strTrim(ruleForm.signatureofShipperorhisAgent).toUpperCase()">
										</div>
										...
										<div id="signature-of-issuing-carrier-or-its-agent" class="horizontal ">
											<input maxlength="50" v-model="ruleForm.carrierSignature" name="Signatory" type="text" class="TextBox" id="Signatory" @change="ruleForm.carrierSignature=strTrim(ruleForm.carrierSignature).toUpperCase()">
...
<script>
	...
	export default {
		...
		inject: ['callbackforInit','setValueInit'],
		created: function() {
			this.afterModify()

			let buttonInfo = window.localStorage.getItem('buttonInfo')
			if (buttonInfo.indexOf('af_button_awbPrint_sendMawb') > -1) {
				this.sendMawbFlag = true;
			}
			if (buttonInfo.indexOf('af_button_awbPrint_finishMawb') > -1) {
				this.finishMawbFlag = true;
			}
		...
		methods: {
			printG() {
				this.$confirm('您将保存并格打主单' + this.ruleForm.awbNumber + ', 是否继续?', '提示', {
          ...
          if (action === 'cancel'){
            ...
            this.$axios.postRequestJSONResponseBlob('/afbase/awbPrint/exportExcel', this.voucher)
              .then(function(response) {
                ...
                downloadElement.download = this.ruleForm.awbNumber + '_FORMAT.xlsx'; // 下载后文件名
			...
			printT() {
				this.$confirm('您将保存并套打主单' + this.ruleForm.awbNumber + ', 是否继续?', '提示', {
          dangerouslyUseHTMLString: true,
          distinguishCancelAndClose: true,
          cancelButtonClass: this.cancelButtonClass,
          cancelButtonText: '导出',
          ...
          if (action === 'cancel') {
            ...
            this.$axios.postRequestJSONResponseBlob('/afbase/awbPrint/exportExcel', this.voucher)
              .then(function (response) {
                ...
                downloadElement.download = this.ruleForm.awbNumber + '.xlsx'; // 下载后文件名
                document.body.appendChild(downloadElement);
                downloadElement.click(); // 点击下载
                document.body.removeChild(downloadElement); // 下载完成移除元素
                window.URL.revokeObjectURL(href); // 释放掉blob对象
          ...
          function extracted() {
            this.$axios.post("/afbase/awbPrint/sendAmsData/"+type+"/"+awbNumber+"/" + (letterId || '')).then(function (response) {
              ...
              if (response.data.code == 0) {
                ...
                this.openError(response.data.messageInfo || "发送AMS失败!");
              }
            }.bind(this));
          }
      ...
      sendAms() {//电子运单 20201116
        ...
        this.$confirm('您将保存并打印运单' + this.ruleForm.awbNumber + ', 是否继续?', '提示', {
          ...
          this.$axios.get('/afbase/aforder/getShippingData/AE_DZ_AWB').then(function(response) {
            if (response.data.code == 0 && response.data.data) { //电子运单
              var awbPrintId = this.ruleForm.awbPrintId;
              this.$axios.get("/afbase/awbPrint/sendAmsCheckHasSend/" + awbPrintId).then(function(response) {//校验是否已发送
                var hasSend='';
                var p_letterId='';
                var p_awbNumber='';
                function extracted() {
                  this.checkSendAms(hasSend,p_awbNumber,p_letterId);
                }
                function extracted2() {
                  if (awbType == 0) {//主单
                    hasSend = 'mwb';
                    this.$confirm('是否同时发送主单下所有分单？', '提示', {
                      confirmButtonText: '是',
                      cancelButtonText: '否',
                      type: 'warning',
                      center: true
                    ...
                    hasSend = 'hawb';
                    p_letterId = awbPrintId;
                    extracted.call(this);
                  }
                }
                if (response.data.code == 0) {
                  var obj = response.data.data.awbPrint;
                  var awbType = obj.awbType;
                  p_awbNumber = obj.awbNumber;
                  if (!response.data.data.hasSend) { //未发送过舱单
                    extracted2.call(this);
                  } else { //已发送过舱单
                    this.$confirm('运单已经发送，是否将再次发送？', '提示', {
                      confirmButtonText: '确定',
                      cancelButtonText: '取消',
                      type: 'warning',
                      center: true
      ...
      openError1(info) {
        this.$notify({
          ...
          position: 'bottom-right'
			...
			afterModify() {
				this.$axios.get2('/afbase/awbPrint/view', {
					awbPrintId: this.frow.awbPrintIdCopy
				}).then((response) => {
          ...
          if(response.data.data.afOrderShipperConsignee2 == null){
			...
			sendCheckNotNull() {
				if (this.ruleForm.afOrderShipperConsignee2.cityCode == null || this.ruleForm.afOrderShipperConsignee2.cityCode == '') {
					this.openError("请录入收货人城市代码")
					return false
				}
				if (this.ruleForm.afOrderShipperConsignee1.cityCode == null || this.ruleForm.afOrderShipperConsignee1.cityCode == '') {
					this.openError("请录入发货人城市代码")
					return false
				}
				...
				if (this.ruleForm.goodsDescription == null || this.ruleForm.goodsDescription == '') {
					this.openError("请录入品名")
					return false
				}
				if (this.ruleForm.rateCharge == null || this.ruleForm.rateCharge == '') {
					this.openError("请录入单价")
					return false
				}
				if (this.ruleForm.carrierSignature == null || this.ruleForm.carrierSignature == '') {
					this.openError("请录入签名")
					return false
				}
				return true
			},
			...
			doSend(sendType,hasMwb,orderUUID,letterIds) {
				this.loading = true
				this.$axios.post("/afbase/send/doEAWB_"+sendType+"/"+hasMwb+"/"+orderUUID+'/'+letterIds).then(function (response) {
		            if (response.data.code == 0) {
		                if(response.data.data.status=='success'){
		                  this.openSuccess('发送成功');
			            }else{
			...
			doSelectSend(sendType) {
				if (!this.sendCheckNotNull()) {
					return
				}
				var orderUUID=this.ruleForm.orderUuid;
				////
				this.$axios.get('/afbase/awbPrint/hawbListByOrderUuid/' + orderUUID).then((response) => {
					var hasMwb='mwb';
					var letterIds='';
					if(response.data.code == 0) {
						var dataRow = response.data.data
			            if(dataRow.length == 0){
			                this.doSend(sendType,hasMwb,orderUUID,letterIds)
			            }else{
			            	this.$confirm('请选择发送内容', '提示', {
							confirmButtonText: '主单',
							cancelButtonText: '主单与分单',
							type: 'warning',
							center: true
							}).then(() => {
								this.doSend(sendType,hasMwb,orderUUID,letterIds)
								...
								for(var i = 0; i < dataRow.length; i++) {
				            	 	if(letterIds == '') {
						              letterIds = dataRow[i].awbPrintId;
						            } else {
						              letterIds = letterIds + ',' + dataRow[i].awbPrintId;
						            }
			            	 	}
			            	 	this.doSend(sendType,hasMwb,orderUUID,letterIds);
							});
			...
			doSave(sendType) {
				...
				this.$axios.put('/afbase/awbPrint/mawb', this.ruleForm)
					.then(function(response) {
						if (response.data.code == 0) {
							this.callbackforInit('mawb');
							this.afterModify();
							this.doSelectSend(sendType);
						} else {
			...
			checkSend(sendType) {
				this.$confirm('您将保存并发送主单' + this.ruleForm.awbNumber + ', 是否继续?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning',
					center: true
			...
			awbFinish() {
				...
				this.ruleForm.awbNumber = this.ruleForm.awbNumberPrefix + "-" + this.ruleForm.awbNumberSuffix
				this.loading1 = true
				this.$axios.put('/afbase/awbPrint/finish/mawb', this.ruleForm)
					.then(function(response) {
						if (response.data.code == 0) {
							this.openSuccess("修改成功！");
							this.callbackforInit('mawb');
