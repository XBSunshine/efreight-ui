<template>
	<el-dialog title="服务 - 修改" width="1300px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" append-to-body>
		<div id="table-form">
			<el-table :data="data1" :header-cell-style="{background:'oldlace'}" :row-class-name="tableRowClassName">
				...
				<el-table-column label="收款客户" width="340">
					<template slot-scope="scope">
						<el-select :ref="'receiveSelectcustomerName'+scope.row.index" @keyup.native.enter="nextFocus($event,scope.row,'customerName','receive')" :default-first-option=true v-model="scope.row.customerName" filterable placeholder="请选择应收服务" @change="setCoop" v-if="!(scope.row.debitNoteId!=null || scope.row.financialDate!=null)" style="width:330px;">
				...
				<el-table-column label="数量" width="110px">
					<template slot-scope="scope">
						<el-input @keyup.native.enter="nextFocus($event,scope.row)" v-model="scope.row.incomeQuantity" @input="formatToFloat('incomeQuantity')" maxlength="8" @change="setIncomeAmount" v-if="!(scope.row.debitNoteId!=null || scope.row.financialDate!=null)"></el-input>
				...
				<el-table-column label="单价" width="97px">
					<template slot-scope="scope">
						<el-input @keyup.native.enter="nextFocus($event,scope.row)" v-model="scope.row.incomeUnitPrice" @input="formatToFloat('incomeUnitPrice')" maxlength="11" @change="setIncomeAmount" v-if="!(scope.row.debitNoteId!=null || scope.row.financialDate!=null)"></el-input>
				...
				<el-table-column label="币种" width="97px">
					<template slot-scope="scope">
						<el-select :ref="'receiveSelectincomeCurrency'+scope.row.index" @keyup.native.enter="nextFocus($event,scope.row,'incomeCurrency','receive')" :default-first-option=true v-model="scope.row.incomeCurrency" filterable placeholder="选择" @change="setIncomeExchangeRate" v-if="!(scope.row.debitNoteId!=null || scope.row.financialDate!=null)">
				...
				<el-table-column label="应收金额">
					<template slot-scope="scope">
						<el-input name="disabled" v-model="scope.row.incomeAmount" @input="formatToFloat('incomeAmount')" maxlength="11" disabled></el-input>
					</template>
				</el-table-column>
