        ...
        <el-col class="elementWidth">
          <el-form-item>
			...
			<el-row>
              ...
              <el-select slot="suffix"  clearable @change="sychronizeWareAndStore('update')"  remote :filter-method="searchAirport" filterable  :loading="loading" v-model="ruleForm.departureStation"
			...
			<el-row v-if="frow.viewFlag">
				<el-form-item>
					<el-button :loading="loading" size="small" type="primary" @click="submitForm4('ruleForm')">修改</el-button>
				</el-form-item>
			</el-row>
		</el-form>
	...
	export default {
      ...
      submitForm4(formName) {
          if(this.ruleForm.signState === 1){
			...
			submitForm(formName) {
			  //如果 已签单 （签单表 签单状态 =1） ，则 不允许修改 服务产品
        if(this.ruleForm.signState === 1 && this.ruleForm.businessProductOld !== this.ruleForm.businessProduct){
            this.openError('当前订单已签单，服务产品不可修改。')
            return
        }
            ...
            this.ruleForm.rountingSign = localStorage.getItem("rountingSign")
            this.ruleForm.rountingSignBusinessProduct = localStorage.getItem("rountingSignBusinessProduct")
						this.$axios.post2('/afbase/aforder/doUpdate', this.ruleForm)
							.then(function(response) {
          ...
          this.workgroups = response.data.data;
			...
			this.$axios.get('/afbase/aforder/view/' + this.frow.orderId)
				.then(function(response) {
					...
					this.ruleForm = response.data.data;
          this.ruleForm.businessProductOld = this.ruleForm.businessProduct
					if (!this.ruleForm.msrCurrecnyCode) {
						this.ruleForm.msrCurrecnyCode = 'CNY';
					}
					if (this.ruleForm.awbId) {
