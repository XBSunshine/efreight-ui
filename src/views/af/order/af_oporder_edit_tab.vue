    ...
    >
          ...
          <el-form-item prop="departureStation">
            <el-input
             class="widthWithThreeWithXing"
                ...
                @change="sychronizeWareAndStore('update')"
            >
              <template slot="prepend">
      ...
      <div style="padding: 0px 0px 0px 30px">
        ...
        <el-row v-show="showFlag1 && ifHaveAwbNumber">
          ...
          >
            <font style="font-size: medium; color: red"
              >当前业务已制主单，若制单信息有变，请到制单页面调整</font
            >
          </el-col>
        </el-row>
        ...
        <el-row v-show="showFlag1">
          <el-row v-show="showFlag1 && ifHaveHawbNumber">
            ...
            >
              <font style="font-size: medium; color: red"
                >系统检测到，当前业务已制分单，若分单的制单信息有变，请到制单页面调整</font
              >
            </el-col>
          </el-row>
              ...
              <el-table-column align="center" label="分单目的港" width="130">
                <template slot-scope="scope">
                  <el-input
                    ...
                    @change="
                      scope.row.arrivalStation = strTrim(
                        scope.row.arrivalStation
                      )
                        .toUpperCase()
                        .substr(0, 3)
                    "
      ...
      <div style="padding: 0px 0px 0px 30px">
        ...
        <el-row v-show="showFlag2">
          <el-col class="elementWidth" style="margin-left: 50px">
            <el-form-item>
              <el-input
                v-if=" this.outfieldDeliveryHandlingCompanyOptions.length > 0 && this.ruleForm.departureStation"
                style="width: 268px"
                clearable
                placeholder="请输入"
              >
          ...
          <el-col class="elementWidth" >
              <el-form-item>
                <el-input
                  v-if="this.wareNameOptions.length > 0 &&this.ruleForm.departureStation"
                  style="width: 268px"
                  clearable
                  placeholder="请输入"
                >
...
export default {
  ...
  methods: {
    ...
    setValueold(rrow) {
      ...
      this.ruleForm.salesName = rrow[0].salesName;
      this.ruleForm.servicerName = rrow[0].servicerName;
      ...
      this.$axios.get("/afbase/coopProjectContacts/page?" + param).then(
        function (response) {
      ...
      if (messageInfo) {
        this.$confirm(messageInfo, "提示", {
          ...
          .then(() => {
            ...
            this.$axios
              .get(
                "/afbase/afoporder/selectOrderStatus?node_name=财务锁账&order_uuid=" +
                  this.frow.orderUuid
              )
              .then(
                function (response) {
    ...
    submitForm3(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$axios
            .post2("/afbase/afoporder/doUpdate", this.ruleForm)
            .then(
              function (response) {
                if (response.data.code == 0) {
                  this.openSuccess();
                  if (
                    localStorage.getItem("orderSaveClosePage") &&
                    localStorage.getItem("orderSaveClosePage") == "true"
                  ) {
                    this.handleClose();
                  } else {
                    this.$axios
                      .get("/afbase/afoporder/view/" + this.frow.orderId)
                      .then((response) => {
    ...
    formatToFloat(item) {
      if (item == "awbNumber") {
        ...
        this.letters.params.planWeight = this.letters.params.planWeight.replace(
          /[^0123456789.]/g,
          ""
        );
    ...
    setBuildUpCompanyOptions(departureStation, flag) {
      if (this.ruleForm.departureStation) {
        this.$axios
    ...
    setOutfieldDeliveryHandlingCompanyOptions(departureStation, flag) {
      if (this.ruleForm.departureStation) {
        this.$axios
    ...
    setwareHouseCpmpanyOptions(departureStation,flag) {
      if (this.ruleForm.departureStation) {
        this.$axios
    ...
    sychronizeWareAndStore(flag) {
      ...
      this.setBuildUpCompanyOptions(this.ruleForm.departureStation, flag);
      this.setOutfieldDeliveryHandlingCompanyOptions(this.ruleForm.departureStation,flag);
      this.setgoodsTypeOptions(this.ruleForm.departureStation,flag);
      this.setwareHouseCpmpanyOptions(this.ruleForm.departureStation,flag);
      // this.checkFlag();
    ...
    sendFHL(row) {
      ...
      if (this.awbNumber) {
        this.$confirm(
          "此操作将发送分单预录入(分单号：" + row.hawbNumber + "）, 是否继续?",
          "注意",
          {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
  ...
  created() {
    ...
    this.$axios.get("/afbase/aforder/selectOrderStatus?node_name=货物出重&order_uuid=" +this.frow.orderUuid)
    .then(
        function (response) {
    ...
    this.$axios
      .get("/afbase/awbPrint/hawbListByOrderUuid/" + this.frow.orderUuid)
      .then(
        function (response) {
    ...
    this.$axios
      .get("/afbase/awbPrint/viewByOrderUuid/" + this.frow.orderUuid)
      .then(
        function (response) {
    ...
    this.$axios.get("/afbase/afoporder/view/" + this.frow.orderId).then(
      function (response) {
