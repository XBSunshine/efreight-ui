<template>
	<div class="orderEditPage">
		<el-form :disabled="!frow.viewFlag" :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
			<el-row>
				...
				<el-col class="elementWidth">
					<el-form-item prop="coopName">
						<el-input readOnly v-model="ruleForm.coopName" style="width:615px;margin-right: 15px;">
			...
			<el-row>
				<el-col class="elementWidth">
					<el-form-item prop="awbNumber">
						<el-input v-model="ruleForm.awbNumber" clearable maxlength="12" auto-complete="off" @input="formatToFloat('awbNumber')" @change="changeAwbNumber" :disabled="disabledFlag" style="width:295px;margin-right: 15px;">
							<template slot="prepend">
								<span>主运单号</span>
							</template>
			...
			<el-row>
				<el-col class="elementWidth">
					<el-form-item prop="departureStation">
						<el-input class="widthWithThreeWithXing" clearable v-model="ruleForm.departureStation" @change="sychronizeWareAndStore('update')" auto-complete="off" maxlength="4" style="width: 295px;margin-right: 15px;" @input="ruleForm.departureStation=strTrim(ruleForm.departureStation).toUpperCase()">
							<template slot="prepend">
								<font style="color: red;">*</font>
								<span>始发港</span>
							</template>
				...
				<el-col class="elementWidth">
					<el-form-item prop="arrivalStation">
						<el-input class="widthWithThreeWithXing arrivalStationForAESave" clearable v-model="ruleForm.arrivalStation" @input="ruleForm.arrivalStation=strTrim(ruleForm.arrivalStation).toUpperCase()" auto-complete="off" maxlength="4" style="width: 295px;margin-right: 15px;" >
							<template slot="prepend">
								<font style="color: red;">*</font>
								<span>目的港</span>
							</template>
				...
				<el-col class="elementWidth">
					<el-form-item prop="expectFlight">
						<el-input class="widthWithThree" clearable v-model="ruleForm.expectFlight" @change="ruleForm.expectFlight=strTrim(ruleForm.expectFlight).toUpperCase()" maxlength="8" auto-complete="off" style="width: 295px;margin-right: 15px;">
							<template slot="prepend">
								<span>航班号</span>
							</template>
						</el-input>
			...
			<el-row>
				<el-col class="elementWidth">
					<el-form-item prop="transitStation">
						<el-input class="widthWithThreeWithXing" v-model="ruleForm.transitStation" clearable @input="ruleForm.transitStation=strTrim(ruleForm.transitStation).toUpperCase()" auto-complete="off" maxlength="4" style="width: 295px;margin-right: 15px;" >
							<template slot="prepend">
								<span>中转港1</span>
							</template>
						</el-input>
				...
				<el-col class="elementWidth">
					<el-form-item prop="transitStation2">
						<el-input class="widthWithThreeWithXing" v-model="ruleForm.transitStation2" clearable @input="ruleForm.transitStation2=strTrim(ruleForm.transitStation2).toUpperCase()" auto-complete="off" maxlength="4" style="width: 295px;margin-right: 15px;">
							<template slot="prepend">
								<span>中转港2</span>
							</template>
						</el-input>
				...
				<el-col class="elementWidth">
					<el-form-item prop="goodsSourceCode">
						<el-input class="widthWithThree" v-model="ruleForm.goodsSourceCode" clearable @change="ruleForm.goodsSourceCode=strTrim(ruleForm.goodsSourceCode).toUpperCase()" auto-complete="off" maxlength="5" style="width: 295px;margin-right: 15px;">
							<template slot="prepend">
								<span>货源地</span>
							</template>
						</el-input>
			...
			<el-row>
				...
				<el-col class="elementWidth">
					<el-form-item prop="planWeight">
						<el-input class="widthWithFourWithXing" @input="formatToFloat('planWeight')" maxlength="10" @change="setPlanChargeWeight();formatDecimalPlaces('planWeight')" v-model="ruleForm.planWeight" auto-complete="off" style="width:295px;margin-right: 15px;" clearable>
							<template slot="prepend">
								<font style="color: red;">*</font>
								<span>预报毛重</span>
							</template>
			...
			<div style="padding:0px 0px 0px 30px;display:none;">
				<span>价格信息</span>
				<a style="color: #409EFF;font-size: 12px;cursor: pointer;" v-if=showFlag3 v-on:click="showFlag3=false">收起<i class="el-icon-arrow-down"></i></a>
				<a style="color: #409EFF;font-size: 12px;cursor: pointer;" v-if=!showFlag3 v-on:click="showFlag3=true">展开<i class="el-icon-arrow-up"></i></a>
			...
			<div style="padding:0px 0px 0px 30px">
				...
				<a style="color: #409EFF;font-size: 12px;cursor: pointer;" v-if=showFlag1 v-on:click="showFlag1=false">收起<i class="el-icon-arrow-down"></i></a>
				<a style="color: #409EFF;font-size: 12px;cursor: pointer;" v-if=!showFlag1 v-on:click="showFlag1=true">展开<i class="el-icon-arrow-up"></i></a>
				<el-row v-show="showFlag1&&ifHaveAwbNumber">
					<el-col class="elementWidth" style="margin-top: 10px;margin-left: 10px;margin-bottom: 10px;">
						<font style="font-size: medium;color: red;">当前业务已制主单，若制单信息有变，请到制单页面调整</font>
					</el-col>
				</el-row>
				...
				<el-row v-show="showFlag1">
					<el-row v-show="showFlag1&&ifHaveHawbNumber">
						<el-col class="elementWidth" style="margin-top: 10px;margin-left: 10px;margin-bottom: 10px;">
							<font style="font-size: medium;color: red;">系统检测到，当前业务已制分单，若分单的制单信息有变，请到制单页面调整</font>
						</el-col>
					</el-row>
					...
					<el-col class="elementWidth" style="margin-left: 50px;margin-top: 20px;">
						<el-table :data="ruleForm.shipperLetters" stripe border style="width:1103px">
							...
							<el-table-column align="center" label="分运单号" width="150">
								<template slot-scope="scope">
									<el-input size="small" v-model="scope.row.hawbNumber" @input="scope.row.hawbNumber=scope.row.hawbNumber.toUpperCase()" maxlength="12" clearable></el-input>
								</template>
							</el-table-column>
							...
							<el-table-column align="center" width="80" v-if="frow.viewFlag">
								<template slot-scope="scope">
									<el-button type="text" size="mini" @click="shipperLetterEdit(scope.$index)">详情</el-button>
									<el-button type="text" size="mini" @click="sendFHL(scope.row)" v-if="scope.row.slId">发送</el-button>
								</template>
							</el-table-column>
			...
			<div style="padding:0px 0px 0px 30px">
				<span>操作详情</span>
				<a style="color: #409EFF;font-size: 12px;cursor: pointer;" v-if=showFlag2 v-on:click="showFlag2=false">收起<i class="el-icon-arrow-down"></i></a>
				<a style="color: #409EFF;font-size: 12px;cursor: pointer;" v-if=!showFlag2 v-on:click="showFlag2=true">展开<i class="el-icon-arrow-up"></i></a>
					<el-row v-show="showFlag2" style="margin-top: 10px;">
						<el-col class="elementWidth" style="margin-left: 50px;">
							<el-form-item prop="goodsType">
								<el-input style="width:268px;">
									...
									<el-select slot="suffix" v-model="ruleForm.goodsType" clearable placeholder="请选择货物类型" style="width:171px;margin-right: -5px;">
						...
						<el-col class="elementWidth" >
							<el-form-item prop="specialHandleCodeM3">
								<el-input style="width:268px;">
									...
									<el-select slot="suffix" v-model="ruleForm.specialHandleCodeM3" clearable placeholder="南京特货请选择特殊货物代码" style="width:268px;margin-right: -5px;">
										<el-option v-for="item in spshTypeOptions" :key="item.value" :label="item.label" :value="item.value">
										</el-option>
									</el-select>
								</el-input>
							</el-form-item>
						...
						<el-col class="elementWidth" >
							<el-form-item prop="businessType">
								<el-input style="width:268px;">
									...
									<el-select slot="suffix" v-model="ruleForm.businessType" clearable placeholder="请选择业务模式" style="width:171px;margin-right: -5px;">
										<el-option v-for="item in businessTypeOptions" :key="item.code" :label="item.label" :value="item.code">
										</el-option>
									</el-select>
								</el-input>
							</el-form-item>
						...
						<el-col class="elementWidth" v-if="customsCodeFlag">
							<el-form-item prop="transPortMode">
								<el-input style="width:268px;">
									...
									<el-select slot="suffix" v-model="ruleForm.transPortMode" clearable placeholder="请选择运输方式" style="width:171px;margin-right: -5px;">
					...
					<el-row v-show="showFlag2">
						<el-col class="elementWidth" style="margin-left: 50px;">
							<el-form-item prop="tranFlag">
								<el-input style="width:268px;">
									...
									<el-select slot="suffix" v-model="ruleForm.tranFlag" placeholder="请选择出口类型" style="width:171px;margin-right: -5px;">
										<el-option v-for="item in tranFlagOptions" :key="item.value" :label="item.label" :value="item.value">
										</el-option>
									</el-select>
								</el-input>
							</el-form-item>
						...
						<el-col class="elementWidth"  v-if="customsCodeFlag">
							<el-form-item prop="customsCode">
								<el-input class="widthWithSix2" v-model="ruleForm.customsCode" auto-complete="off" clearable maxlength="4" style="width:268px;">
									<template slot="prepend">
										<font style="color: red;">*</font>
										<span>离境地海关代码</span>
									</template>
						...
						<el-col class="elementWidth">
							<el-form-item prop="batteryType">
								<el-input style="width:268px;">
									...
									<el-select slot="suffix" v-model="ruleForm.batteryType" clearable placeholder="请选择电池情况" style="width:171px;margin-right: -5px;">
						...
						<el-col class="elementWidth">
							<el-form-item prop="customsStatusCode">
								<el-input class="widthWithSix2" style="width:268px;">
									...
									<el-select slot="suffix" v-model="ruleForm.customsStatusCode" clearable placeholder="请选择海关代码" style="width:171px;margin-right: -5px;">
					...
					<el-row v-show="showFlag2">
						<el-col class="elementWidth" style="margin-left: 50px;">
							<el-form-item prop="packSize">
								<el-input  v-model="ruleForm.packSize" auto-complete="off" clearable maxlength="32" style="width:268px;">
									<template slot="prepend">
										<span>包裹大小</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth" >
							<el-form-item prop="dangerousType" >
								<el-input style="width:268px;"  @input="ruleForm.dangerousType = ruleForm.dangerousType.toUpperCase()" v-model="ruleForm.dangerousType" auto-complete="off" clearable maxlength="40">
									<template slot="prepend">
										<span>危品类项</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth">
							<el-form-item prop="identificationNo">
								<el-input style="width:546px;"  @input="ruleForm.appraisalNote = ruleForm.appraisalNote.toUpperCase()" v-model="ruleForm.appraisalNote" auto-complete="off" clearable maxlength="200">
									<template slot="prepend">
										<span>鉴定号</span>
									</template>
								</el-input>
					...
					<el-row v-show="showFlag2">
						<el-col class="elementWidth" style="margin-left: 50px;">
							<el-form-item prop="appraisalCompany" >
								<el-input  style="width:546px;" @input="ruleForm.appraisalCompany = ruleForm.appraisalCompany.toUpperCase()" v-model="ruleForm.appraisalCompany" auto-complete="off" clearable maxlength="200">
									<template slot="prepend">
										<span>鉴定单位</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth">
							<!-- <el-form-item >
								<el-input style="width:546px;" class="" @input="ruleForm.appraisalNote = ruleForm.appraisalNote.toUpperCase()" v-model="ruleForm.appraisalNote" auto-complete="off" clearable maxlength="200">
									<template slot="prepend">
										<span>鉴定情况</span>
									</template>
								</el-input>
							...
							<el-form-item>
								<el-input style="width:546px;" v-model="ruleForm.airborneDocument" auto-complete="off" clearable maxlength="200">
									<template slot="prepend">
										<span>随机文件</span>
									</template>
								</el-input>
					...
					<!-- <el-row v-show="showFlag2" >
						<el-col class="elementWidth" style="margin-left: 50px;">
							<el-form-item>
								<el-input style="width:1100px;" v-model="ruleForm.airborneDocument" auto-complete="off" clearable maxlength="200">
									<template slot="prepend">
										<span>随机文件</span>
									</template>
								</el-input>
					...
					<el-row v-show="showFlag2" >
						<el-col class="elementWidth" style="margin-left: 50px;">
							<el-form-item prop="mainOrderUnit">
								<el-input style="width:268px;" class="" v-model="ruleForm.mainOrderUnit" auto-complete="off" clearable maxlength="32">
									<template slot="prepend">
										<span>主货单位</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth">
							<el-form-item>
								<el-input class="widthWithSix"style="width:268px;">
									<template slot="prepend">
										<span>截单时间</span>
									</template>
									<el-date-picker slot="suffix" v-model="ruleForm.cutoffTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择送达时间" style="width: 193px;margin-right: -5px;">
									</el-date-picker>
								</el-input>
							</el-form-item>
						</el-col>
						<el-col class="elementWidth">
							<el-form-item prop="minTemperature" >
								<el-input style="width:268px;" v-model="ruleForm.minTemperature" auto-complete="off" clearable maxlength="32">
									<template slot="prepend">
										<span>最低温度</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth">
							<el-form-item prop="maxTemperature">
								<el-input style="width:268px;" v-model="ruleForm.maxTemperature" auto-complete="off" clearable maxlength="32">
									<template slot="prepend">
										<span>最高温度</span>
									</template>
								</el-input>
					...
					<el-row v-show="showFlag2">
						<el-col class="elementWidth" style="margin-left: 50px;">
							<el-form-item prop="securitycheckerName" >
								<el-input style="width:268px;" class="widthWithFive"  v-model="ruleForm.securitycheckerName" auto-complete="off" clearable maxlength="40">
									<template slot="prepend">
										<span>报检员姓名</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth">
							<el-form-item prop="securitycheckerCertificate" >
								<el-input style="width:268px;" class="" v-model="ruleForm.securitycheckerCertificate" auto-complete="off" clearable maxlength="32">
									<template slot="prepend">
										<span>报检证号</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth">
							<el-form-item prop="securitycheckerCardId">
								<el-input style="width:268px;" class="widthWithSix2" v-model="ruleForm.securitycheckerCardId" auto-complete="off" clearable maxlength="32">
									<template slot="prepend">
										<span>报检员身份证号</span>
									</template>
								</el-input>
						...
						<el-col class="elementWidth">
							<el-form-item prop="securitycheckerTel" >
								<el-input style="width:268px;" v-model="ruleForm.securitycheckerTel" auto-complete="off" clearable maxlength="20">
									<template slot="prepend">
										<span>联系电话</span>
									</template>
								</el-input>
				...
				<el-row v-show="showFlag2" style="margin-top: 20px;">
					<el-tabs style="width: 1103px;height:375px;margin-left: 50px;" type="border-card" v-model="activeOperationName">
						<el-tab-pane name="first">
							...
							<el-row>										
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="showWordLimit" :disabled="!ruleForm.pickUpDeliveryService" v-model="ruleForm.pickUpTruckNumber" auto-complete="off" style="width:300px;margin-right: 15px;" clearable show-word-limit maxlength="20">
												<template slot="prepend">
													<span>提货车牌号</span>
												</template>
											</el-input>
									...
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="showWordLimit" :disabled="!ruleForm.pickUpDeliveryService" v-model="ruleForm.pickUpDriver" auto-complete="off" style="width:340px;margin-right: 15px;" clearable show-word-limit maxlength="64">
												<template slot="prepend">
													<span>提货司机</span>
												</template>
											</el-input>
									...
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="showWordLimit widthWithSix2" :disabled="!ruleForm.pickUpDeliveryService" v-model="ruleForm.pickUpDriverTel" auto-complete="off" style="width:365px;" clearable show-word-limit maxlength="20">
												<template slot="prepend">
													<span>提货司机联系方式</span>
												</template>
											</el-input>
						...
						<el-tab-pane name="third">
							...
							<el-row>
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="showWordLimit" :disabled="!ruleForm.outfieldService" v-model="ruleForm.outfieldDeliveryDrieverLicense" auto-complete="off" style="width:335px;margin-right: 15px;" clearable show-word-limit maxlength="32">
												<template slot="prepend">
													<span>驾驶证号</span>
												</template>
											</el-input>
									...
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="showWordLimit widthWithSix2" :disabled="!ruleForm.outfieldService" v-model="ruleForm.outfieldDeliveryDriverTel" auto-complete="off" style="width:335px;margin-right: 15px;" clearable show-word-limit maxlength="20">
												<template slot="prepend">
													<span>司机联系方式</span>
												</template>
											</el-input>
									...
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="widthWithSix2" style="width:335px;">
												<template slot="prepend">
													<span>入区预约时间</span>
												</template>
												<el-date-picker :disabled="!ruleForm.outfieldService" slot="suffix" v-model="ruleForm.outfieldDeliveryArriveTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择时间" style="width: 238px;margin-right: -5px;">
												</el-date-picker>
											</el-input>
										</el-form-item>
									</el-col>
							...
							<el-row>
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="showWordLimit" :disabled="!ruleForm.outfieldService" v-model="ruleForm.outfieldDeliveryHandlingCompany" auto-complete="off" style="width:335px;margin-right: 15px;" clearable show-word-limit maxlength="32">
												<template slot="prepend">
													<span>搬运公司</span>
												</template>
											</el-input>
									...
									<el-col class="elementWidth">
										<el-form-item>
											<el-input class="showWordLimit" :disabled="!ruleForm.outfieldService" v-model="ruleForm.transitCode" auto-complete="off" style="width:335px;margin-right: 15px;" clearable show-word-limit maxlength="20">
												<template slot="prepend">
													<span>转关单号</span>
												</template>
											</el-input>
			...
			<el-row v-if="frow.viewFlag">
				<el-form-item>
					<el-button :loading="loading" size="small" type="primary" @click="submitForm('ruleForm')">修改</el-button>
				</el-form-item>
			</el-row>
		</el-form>
...
                ],
				...
				ifHaveHawbNumber: false,
				ifHaveAwbNumber: false,
...
///////
		...
		methods: {
			...
			submitForm(formName) {
				...
				let messageInfo = ''
				if (!this.ruleForm.confirmDensity) {
					let sumPieces = 0
					let sumWeight = 0.0
				...
				if (messageInfo) {
					this.$confirm(messageInfo, '提示', {
						...
						this.loading = true
						this.$axios.get('/afbase/afoporder/selectOrderStatus?node_name=财务锁账&order_uuid=' + this.frow.orderUuid).then(function(
							response) {
			...
			submitForm3(formName) {
				...
				if (this.ruleForm.awbNumber == null) {
				...
				if (this.ruleForm.expectArrival == 'null') {
					this.ruleForm.expectArrival = '';
				}
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$axios.post2('/afbase/afoporder/doUpdate', this.ruleForm)
							.then(function(response) {
								if (response.data.code == 0) {
									...
									if (localStorage.getItem("orderSaveClosePage") && localStorage.getItem("orderSaveClosePage") == 'true') {
										...
										this.$axios.get('/afbase/afoporder/view/' + this.frow.orderId)
											.then((response) => {
												this.ruleForm = response.data.data;
												if (!this.ruleForm.msrCurrecnyCode) {
													this.ruleForm.msrCurrecnyCode = 'CNY';
												}
												if (this.ruleForm.awbId) {
			...
			handleClick(tab, event) {
				if (tab.name == 'first') {
					this.helpDocumentUrl = this.firstUrl;
				} else if (tab.name == 'fourth') {
					this.helpDocumentUrl = this.fourthUrl;
				} else if (tab.name == 'second') {
					this.helpDocumentUrl = this.secondUrl;
				} else if (tab.name == 'fifth') {
					this.helpDocumentUrl = this.fifthUrl;
				} else {
			...
			formatToFloat(item) {
				...
				} else if (item == 'planWeight2') {
					this.letters.params.planWeight = this.letters.params.planWeight.replace(/[^0123456789.]/g, "");
					let t = this.letters.params.planWeight.split('.');
					if (t.length > 1) {
						this.letters.params.planWeight = t[0] + "." + t[1].substr(0, 1);
					}
			...
			checkFlag() {
				...
				this.$axios.post2('/afbase/airport/checkFlag', params)
					.then(function(response) {
						this.customsCodeFlag=response.data.data;
				}.bind(this));
				this.$refs['ruleForm'].clearValidate();
			},
			setPackageTypeOptions(departureStation,flag){
				// if (flag == 'update') {
				// 	this.ruleForm.packageType='';
				// }
				if (this.ruleForm.departureStation) {
					this.$axios.get('/afbase/awb/selectCategoryPro/包装类型/'+departureStation).then(function(response) {
			...
			sychronizeWareAndStore(flag) {
				...
				this.setPackageTypeOptions(this.ruleForm.departureStation,flag);
				// this.checkFlag();
			},
			addHawb() {
			...
			sendFHL(row){
				if (!row.hawbNumber
	            	...
	            	||!row.planWeight
	            	||!row.planVolume
	            	||!row.customsStatusCode
	            	||!row.paymentMethod
	            	||!row.afOrderShipperConsignee2.scPrintRemark
	            	||!row.afOrderShipperConsignee1.scPrintRemark
	            	||!row.goodsNameEn) {
			        this.openError("分单信息不全，请按红色*录入");
			        return;
			    }
				if (this.awbNumber) {
					this.$confirm('此操作将发送分单预录入(分单号：'+row.hawbNumber+'）, 是否继续?', '注意', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning',
					center: true
					}).then(() => {
						this.$axios.post("/afbase/send/doEsdDecleareFHL/"+this.ruleForm.orderUuid+'/'+row.slId).then(function (response) {
				            if (response.data.code == 0) {
			...
			shipperLetterEdit(index) {
				this.ffrow.ifHaveAwbNumber = this.ifHaveAwbNumber
				this.ffrow.ifHaveHawbNumber = this.ifHaveHawbNumber
				if (index || index === 0) {
			...
			changeIsConsol(val) {
				if (val) {
					...
					this.ruleForm.goodsNameEn = this.ruleForm.goodsNameEn.replace(/CONSOL AS PER ATTACHED MANIFEST/g, '')
				}
			},
		...
		created() {
			...
			if (buttonInfo.indexOf('af-oporder-edit') > -1) {
				this.editButtonFlag2 = this.frow.editButtonFlag2;
				this.editButtonFlag = true;
			}
			this.$axios.get('/afbase/aforder/selectCompany').then(function(response) {
			...
			this.$axios.get('/afbase/aforder/selectOrderStatus?node_name=货物出重&order_uuid=' + this.frow.orderUuid).then(function(
				response) {
				this.disabledFlag2 = response.data.data;
			}.bind(this));
			...
			this.$axios.get('/afbase/awb/selectCategory?category=业务模式').then(function(response) {
			...
			this.$axios.get('/afbase/awbPrint/hawbListByOrderUuid/' + this.frow.orderUuid).then(function(response) {
				if (response.data.code == 0 && response.data.data.length > 0) {
					this.ifHaveHawbNumber = true
				}
			}.bind(this));
			this.$axios.get('/afbase/awbPrint/viewByOrderUuid/' + this.frow.orderUuid).then(function(response) {
				if (response.data.code == 0 && response.data.data) {
					this.ifHaveAwbNumber = true
				}
			}.bind(this));
			...
			this.$axios.get('/afbase/afoporder/view/' + this.frow.orderId)
				.then(function(response) {
					this.queryContactsList2(response.data.data.coopId);
					this.ruleForm = response.data.data;
					this.awbNumber=this.ruleForm.awbNumber;
					if (!this.ruleForm.msrCurrecnyCode) {
						this.ruleForm.msrCurrecnyCode = 'CNY';
					}
					if (this.ruleForm.awbId) {
