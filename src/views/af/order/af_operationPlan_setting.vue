...
<script>
  import columns from './af_operationPlan_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_operationPlan_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.findByPage();
    ...
    sortBykey(ary, key) {
      return ary.sort(function (a, b) {
		...
		created() {
      let afOperationPlanInfo = JSON.stringify(columns.afOperationPlanInfo);
      this.oldTableColumns =afOperationPlanInfo;
      this.tableColumns = JSON.parse(afOperationPlanInfo);
      this.tableColumns =  this.sortBykey(this.tableColumns, 'index');
		},
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  operationPlan_list_column_width = window.localStorage.getItem(userId+'_operationPlan_list_column_width');
      //从数据库查询设置信息
      let pageName = '订单操作管理/空运出口/操作计划';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.operationPlan_list_column = response.data.data;

              //如果operationPlan_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(operationPlan_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(operationPlan_list_column_width);
              ...
              if(this.operationPlan_list_column && this.operationPlan_list_column.length>0){
                  let loaclJsonArray = this.operationPlan_list_column;
