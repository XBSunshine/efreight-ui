<template>
	<el-dialog title="账单 - 发送" width="1300px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" append-to-body>
		<div>
			...
			<el-table :span-method="arraySpanMethod" :indent="0" v-loading="loading" stripe :data="data" row-key="debitNoteId" :expand-row-keys="expandRowKeys" border :tree-props="{children: 'children'}">
				...
				<el-table-column v-if="this.frow.businessScope != 'IO' && this.frow.businessScope != 'LC'" align="center" prop="awbNumber" width="110" label="主单号"></el-table-column>
		...
		<div style="margin-top: 10px;margin-bottom: 10px;margin-left: 5px;">
			<el-button size="small" type="primary" @click="sendDebitNote">发送账单</el-button>
		</div>
		<send ref="addMission" v-if="sendVisible" :visible.sync="sendVisible" :frow="ffrow"></send>
	</el-dialog>
</template>
<script>
	import billSend from './af_css_debit_note_send'
	export default {
		props: {
			visible: {
				type: Boolean,
