	...
	<div class="warp-main">
		<el-row class="css_P_form">
                  ...
                  <el-input v-model="query.exitPort" @input="query.exitPort=toUpperCaseValue(query.exitPort)" clearable auto-complete="off" style="width:189px;" maxlength="4" show-word-limit>
                    <template slot="prepend">进境口岸</template>
						...
						<el-row v-show="showFlag">
							<el-col class="elementWidth">
								<el-form-item label-width="10px">
									<el-input style="width:189px;">
										...
										<el-select slot="suffix" clearable v-model="query.salesDep" placeholder="请选择" style="width:120px;margin-right: -5px;">
							...
							<el-col class="elementWidth">
								<el-form-item label-width="10px">
									<el-input v-model="query.servicerName" clearable auto-complete="off" style="width:170px;">
                  ...
                  <el-input class="widthWithThree" style="width:189px;">
                    ...
                    <el-select slot="suffix" v-model="query.workgroupId" filterable placeholder="请选择" clearable style="width: 120px;margin-right: -5px">
                      <el-option v-for="item in workgroupIds" :key="item.workgroupId" :label="item.workgroupName" :value="item.workgroupId">
							...
							<el-col class="elementWidth" v-if="query.businessScope=='AE'">
								<el-form-item label-width="10px">
									<el-input v-model="query.awbFromName" clearable auto-complete="off" style="width:132px;" maxlength="50" show-word-limit>
									</el-input>
								</el-form-item>
							</el-col>
							...
							<el-col class="elementWidth" v-if="businessScopeFlag2 && query.businessScope!='TE' && query.businessScope!='TI' && query.businessScope!='LC'&& query.businessScope!='IO' && query.businessScope!='AI'">
								<el-form-item label-width="10px">
									<el-input style="width:189px;">
										...
										<el-select slot="suffix" v-model="query.businessProduct" filterable placeholder="请选择" clearable style="width: 120px;margin-right: -5px">
							...
							<el-col class="elementWidth">
								<el-form-item label-width="10px">
									<el-input style="width:170px;">
										...
										<el-select slot="suffix" v-model="query.grossProfitStr" filterable clearable placeholder="请选择" style="width: 101px;margin-right: -5px">
          ...
          <el-table-column v-if="item.prop=='workgroupName'" :key="index" :prop="item.prop" :label="item.label" :width="item.width" :align="item.align" :sortable="item.sortable"></el-table-column>
					...
					<el-table-column v-if="item.prop=='mainRoutingGrossProfitWeight' && flagAI" :key="index" :prop="item.prop" label="干线单公斤毛利" :width="item.width" :align="item.align" header-align="center" :sortable="item.sortable" :sort-method="sortMainRoutingGrossProfitWeight"></el-table-column>
...
<script>
	import setVisibleVue from './css_p_r_eport_sale_settle_af/column_setting'
  ...
  import viewVisibleVueTI from '../tc/ti/order/main/order_view.vue'
	...
	import columns from './css_p_r_eport_sale_settle_af/css_p_r_eport_sale_settle_af_column.json'
	...
	export default {
      ...
      queryWorkgroup(businessScope){
          ...
          //中转港1
          if (column.prop == 'transitStation') {
              if (this.query.businessScope != "AE") {
                  indexDelete.push(index);
              }
          }
          //中转港2
          if (column.prop == 'transitStation2') {
              if (this.query.businessScope != "AE") {
                  indexDelete.push(index);
              }
          }
		...
		/*			if (column.prop == 'mainRoutingGrossProfitWeight') {
			...
			cellWidth(newWidth, oldWidth, column, event) {
				...
				if (arrayC && arrayC.length > 0) {
					...
					localStorage.setItem(userId + '_css_p_r_eport_settle_af_list_column_width_sale', JSON.stringify(arrayC))
				}
			},
			setting() {
        ...
        this.query.orderPermission = orderPermission;
				...
				this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
					.then(function(response) {
						...
						this.$axios.post('/afbase/cssPReportSettleAf/saleSettle', this.query).then((response) => {
							this.loading = false
        ...
        this.query.reportType = 'sale';
				this.$axios.post3('/afbase/cssPReportSettleAf/exportExcel', this.query)
					.then((response) => {
						...
						downloadElement.download = '销售结算报表' + '.xls'; // 下载后文件名
