<template>
	<el-dialog v-loading="pageLoading" title="报关单制作 - 出口 - 编辑" fullscreen :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :show-close="false">
		...
		methods: {
			...
			submitForm(formName) {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.put('/afbase/customsDeclaration', this.ruleForm)
							.then((response) => {
						...
						this.$axios.put('/afbase/customsDeclaration', this.ruleForm)
							.then((response) => {
								if (response.data.code == 0) {
									...
									window.opener.queryList()
									this.exportExcel(this.frow.customsDeclarationId)
								} else {
									this.openError(response.data.messageInfo)
									this.buttonLoading = false
								}
			...
			init() {
				...
				this.$axios.get('/afbase/category/queryCategoryByCategoryType/21')
					.then((response) => {
						...
						this.tradeModesTemp = this.tradeModes.slice(0, 10)
						if (this.ruleForm.customsDeclarationId && this.ruleForm.tradeMode && !this.tradeModesTemp.some(item => item.ediCode1 == this.ruleForm.tradeMode)) {
							this.tradeModesTemp = this.tradeModesTemp.concat(this.tradeModes.filter(item => item.ediCode1 == this.ruleForm.tradeMode))
						}
					})
				//征免性质
				this.$axios.get('/afbase/category/queryCategoryByCategoryType/22')
					.then((response) => {
						...
						this.cutModesTemp = this.cutModes.slice(0, 10)
						if (this.ruleForm.customsDeclarationId && this.ruleForm.cutMode && !this.cutModesTemp.some(item => item.ediCode1 == this.ruleForm.cutMode)) {
							this.cutModesTemp = this.cutModesTemp.concat(this.cutModes.filter(item => item.ediCode1 == this.ruleForm.cutMode))
						}
					})
				//贸易国
				...
				this.$axios.get('/afbase/airport/queryNationWithNationCodeThreeIsNotNull')
					.then((response) => {
						...
						this.countryDepartureArrivalsTemp = this.countrys.slice(0, 10)
						this.countryDepartureArrival1sTemp = this.countrys.slice(0, 10)
						if (this.ruleForm.customsDeclarationId && this.ruleForm.countryTrade && !this.countryTradesTemp.some(item => item.nationCodeThree == this.ruleForm.countryTrade)) {
							this.countryTradesTemp = this.countryTradesTemp.concat(this.countrys.filter(item => item.nationCodeThree == this.ruleForm.countryTrade))
						}
						if (this.ruleForm.customsDeclarationId && this.ruleForm.countryDepartureArrival && !this.countryDepartureArrivalsTemp.some(item => item.nationCodeThree == this.ruleForm.countryDepartureArrival)) {
							this.countryDepartureArrivalsTemp = this.countryDepartureArrivalsTemp.concat(this.countrys.filter(item => item.nationCodeThree == this.ruleForm.countryDepartureArrival))
						}
						if (this.ruleForm.customsDeclarationId && this.ruleForm.countryDepartureArrival1 && !this.countryDepartureArrival1sTemp.some(item => item.nationCodeThree == this.ruleForm.countryDepartureArrival1)) {
							this.countryDepartureArrival1sTemp = this.countryDepartureArrival1sTemp.concat(this.countrys.filter(item => item.nationCodeThree == this.ruleForm.countryDepartureArrival1))
						}
					})
				//离境口岸
