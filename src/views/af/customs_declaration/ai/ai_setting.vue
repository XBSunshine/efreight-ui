...
<script>
	import columns from './ai_column.json'
	export default {
		props: {
			visible: {
				type: Boolean,
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_customs_declaration_list_column_width'); //清除掉页面拖动字段宽度的缓存
			...
			sureSave() {
				...
				if (this.multipleSelection && this.multipleSelection.length > 0) {
					...
					this.$axios.post2('/hrs/user/saveUserPageSet', {
							...
							pageName: '订单操作管理/空运进口/进口报关单'
						})
						.then(function(response) {
							if (response.data.code == 0) {
								this.columnSetCallback()
								this.handleClose();
							} else {
		...
		created() {
			let aiInfo = JSON.stringify(columns.columns);
			this.oldTableColumns = aiInfo;
			//从数据库查询设置信息
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=订单操作管理/空运进口/进口报关单")
				.then(function(response) {
		...
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let ai_list_column_width = window.localStorage.getItem(userId + '_customs_declaration_list_column_width');
			//从数据库查询设置信息
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=订单操作管理/空运进口/进口报关单")
				.then(function(response) {
					this.ai_list_column = response.data.data;
