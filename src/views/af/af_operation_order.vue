					...
					<el-col class="elementWidth">
						<el-form-item label-width="10px">
							<el-input style="width:150px;">
								...
								<el-select slot="suffix" v-model="query.incomeRecorded" placeholder="请选择" style="width:81px;margin-right: -5px;">
					...
					<el-col class="elementWidth">
						<el-form-item label-width="10px">
							<el-input style="width:150px;">
								...
								<el-select slot="suffix" v-model="query.costRecorded" placeholder="请选择" style="width:81px;margin-right: -5px;">
									<el-option label="全部" value=""></el-option>
									<el-option label="是" value="true"></el-option>
									<el-option label="否" value="false"></el-option>
								</el-select>
						...
						<el-dropdown-menu slot="dropdown">
							...
							<el-dropdown-item>
								<el-dropdown placement="right-start" @command="handleCommand" @visible-change="handleChange(scope.row)">
									<span class="el-dropdown-link">交运操作<i class="el-icon-arrow-right el-icon--right"></i></span>
									<el-dropdown-menu slot="dropdown">
										<el-dropdown-item command="sendESD" >发送预录入</el-dropdown-item>
										<el-dropdown-item command="shippingSend" v-if="shippingSendButtonFlag">预配申报</el-dropdown-item>
										<el-dropdown-item command="sendOneDecleare" >一键申报</el-dropdown-item>
...
<script>
	import setVisibleVue from './order/af_oporder_order_setting'
	import saveVisibleVue from './order/af_oporder_base_save'
	import editVisibleVue from './order/af_oporder_edit_tabs'
	// import editVisibleVue from './order/af_order_base_edit'
	import viewVisibleVue from './order/af_oporder_edit_tabs'
	...
	import CopyOrderSave from './order/af_oporder_copyOrderSave'
	import {
		orderDisplayRule
	...
	import columns from './order/af_oporder_order_column.json'
	...
	export default {
		...
		methods: {
			...
			formatter_column(row, column) {
				...
				if (column.property === "storagePieces") {
					let storagePieces = "-";
					if (row.storagePieces || row.storagePieces == 0) {
						storagePieces = this.getNumber4(row.storagePieces);
					}
					return storagePieces;
				}
				if (column.property === "storageWeight") {
					let storageWeight = "-";
					if (row.storageWeight || row.storageWeight == 0) {
						storageWeight = this.getNumber4(row.storageWeight);
					}
					return storageWeight;
				}

				if (column.property === "tranFlag") {
					if (row.tranFlag=='0') {
						return "本关"
					}else if(row.tranFlag=='1'){
						return "转关"
					}else if(row.tranFlag=='2'){
						return "大通关"
					}
				}
			},
			...
			handleCommand(command) {
				if (command == 'edit') {
				...
				} else if (command == 'sendESD') {
					this.doSendESD(this.currRow)
				} else if (command == 'deleteESD') {
					this.doDeleteESD(this.currRow)
				} else if (command == 'exitCard') {
					this.doExitCard(this.currRow)
				}else if (command == 'printGoodsName') {
					this.doPrintGoodsName(this.currRow)
				}else if (command == 'sendEAWB_WH') {
					this.doSendEAWB_WH(this.currRow)
				} else if (command == 'sendOneDecleare') {
					this.doSendOneDecleare(this.currRow)
				} else if (command == 'uninstall') {
					this.checkLookDada(this.currRow)
					...
					this.Mft2201_Decleare(this.currRow)
			...
			printAirCargoManifest(row) {
				this.$axios.post3('/afbase/afoporder/airCargoManifestPrint/' + row.orderId).then((response) => {
			...
			doPrintGoodsName(row) {
				this.$axios.post4('/afbase/afoporder/doPrintGoodsName', row).then(function(response) {
					var blob = new Blob([response.data], {
						type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8'
					});
					var downloadElement = document.createElement('a');
					var href = window.URL.createObjectURL(blob); // 创建下载的链接
					downloadElement.href = href;
					downloadElement.download = '货物品名清单' + '.xlsx'; // 下载后文件名
					document.body.appendChild(downloadElement);
					downloadElement.click(); // 点击下载
					document.body.removeChild(downloadElement); // 下载完成移除元素
					window.URL.revokeObjectURL(href); // 释放掉blob对象
					...
					// console.log(error);
					// alert(error.response.data.messageInfo)
					this.openError("无货物品名清单数据");
				}.bind(this));
			},
			doExitCard(row) {
				if (!row.awbNumber) {
		          this.openError1("未配主单号，不能退库");
		          return;
		        }
				if (!row.storageTime) {
					this.openError1("未入库，不能退库");
					return;
				}
				this.$axios.post4('/afbase/afoporder/exitCard',row).then((response) => {
					var blob = new Blob([response.data], {
						type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8'
					});
					var downloadElement = document.createElement('a');
					var href = window.URL.createObjectURL(blob); // 创建下载的链接
					downloadElement.href = href;
					downloadElement.download = '退库出门证' + '.xls'; // 下载后文件名
					document.body.appendChild(downloadElement);
					downloadElement.click(); // 点击下载
					document.body.removeChild(downloadElement); // 下载完成移除元素
					window.URL.revokeObjectURL(href); // 释放掉blob对象
			...
			printOrderLetter(row) {
				...
				let orgId = window.localStorage.getItem("orgId");
				let userId = window.localStorage.getItem("userId");
				let url = "http://" + hostName + "/afbase/afoporder/printOrderLetter";
				let orderUuid = row.orderUuid;
				this.openPostWindow(url, orgId, orderUuid, userId);*/
				let userId = window.localStorage.getItem("userId");
				this.$axios.post('/afbase/afoporder/printOrderLetter1/' + row.orderUuid + '/' + userId).then((response) => {
					window.open(response.data.data)
				}).catch(function(error) {
					console.log(error);
				});
			...
			shippingMake(row) {
				...
				this.$axios.get('/afbase/afoporder/getShippingData/AE_CD_POST_MAWB ').then(function(response) {
					if (response.data.data) { //找到有效数据
						this.$axios.get("/afbase/afoporder/shippingBillDataCheck/AE_CD_POST_MAWB/all/" + orderUUID).then(function(response) {
							function extracted() {
								this.$axios.get("/afbase/afoporder/shippingBillData/" + orderUUID).then(function(response) {
									if (response.data.code == 0) {
			...
			findByPage() {
				...
				this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
					.then(function(response) {
						...
						this.loading = true
						this.$axios.get2("/afbase/afoporder/page?size=" + this.pageConf.pageSize + "&current=" + this.pageConf.pageCode, this.query)
							.then(function(response) {
						...
						this.$axios.get2("/afbase/afoporder/getTatol", this.query)
							.then(function(response) {
			...
			findByPage2(current) {
				this.$axios.get2("/afbase/afoporder/page?size=" + this.pageConf.pageSize + "&current=" + current, this.query)
					.then(function(response) {
						this.loading = false
						this.data1 = response.data.data.records;
						this.pageConf.totalPage = response.data.data.total;
			...
			checkLookDada(row){
				let params={
                  'currentPage': 1,
                  'pageSize': 10,
          'mawbCode': row.awbNumber
                }
                this.$axios.post('/afbase/fhl/queryLookList',params)
                .then(function(response) {
                	if (response.data.code == 1) {
                		if ('没有配置，请联系管理员'==response.data.messageInfo) {
                			this.doDelete(row);
                			return
                		}else{
                			this.openError1(response.data.messageInfo)
                    ...
                    if (response.data.data.code=='01') {
                    	if (response.data.data.dataList.length>0) {
                    		var lookData=response.data.data.dataList[0];
                    		if (lookData.mft9999Recv){
								this.openError("该运单已放行，请先退关、退库、删除预录入，再卸载主单");
							}else if (lookData.mft3201Recv||lookData.mft2201Status){
								this.openError("该运单已申报舱单数据，请先删除舱单、删除预录入，再卸载主单");
							}else if (lookData.acFohTime){
								this.openError("该运单已入仓，请先退库、删除预录入，再卸载主单");
							}else if (lookData.sliStatus){
								this.openError("该运单已预录入，请先删除预录入，再卸载主单");
							}else{
								this.doDelete(row);
							}
			...
			doUninstall(row) {
				this.$axios.get('/afbase/afoporder/selectOrderStatus?node_name=货物出重&order_uuid=' + row.orderUuid).then(function(response) {
					if (!response.data.data) {
			...
			exportExcelSure() {
				this.$axios.post3('/afbase/afoporder/exportExcel', this.query)
					.then(function(response) {
						var blob = new Blob([response.data], {
							type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8'
						});
						...
						var href = window.URL.createObjectURL(blob); // 创建下载的链接
						downloadElement.href = href;
						downloadElement.download = '操作订单列表' + '.xls'; // 下载后文件名
						document.body.appendChild(downloadElement);
						downloadElement.click(); // 点击下载
						document.body.removeChild(downloadElement); // 下载完成移除元素
						window.URL.revokeObjectURL(href); // 释放掉blob对象
			...
			awbSubmit(row) {
				this.$axios.post('/afbase/afoporder/awbSubmit/' + row.orderUuid)
					.then(function(response) {
						window.open(response.data.data)
					}.bind(this)).catch(function(error) {
						let errorinfo = error.response.data.messageInfo;
				...
				if (loaclJsonArray) {
					for (let i = 0; i < loaclJsonArray.length; i++) {
						if (this.data2.length > 0) {
							...
							if (loaclJsonArray[i].prop === "storagePieces") {
								sums[i + 1] = this.getNumber4(this.data2[0].storagePieces);
							}
							// if (loaclJsonArray[i].prop === "storageWeight") {
							// 	sums[i + 1] = this.getNumber4(this.data2[0].storageWeight);
							// }
						}
					}
				}
			...
			waybillMake() {
				if (this.currRow.awbNumber) {
					// this.$axios.get('/afbase/afoporder/getShippingData/AE_DZ_POST_MAWB').then(function(response) {
			...
			doDeleteESD() { //发送舱单20200827
				if (this.currRow.awbNumber) {
					this.$confirm('此操作将删除预录入, 是否继续?', '注意', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning',
					center: true
					}).then(() => {
						this.$axios.post("/afbase/send/doEawbPreDelete/"+this.currRow.awbNumber+"/"+this.currRow.orderUuid).then(function (response) {
				            if (response.data.code == 0) {
      ...
      doMft2201_Delete(){
        if (this.currRow.awbNumber) {
          ...
          this.openError1("未配主单号，不能删除");
			...
			doSendESD() { //发送舱单20200827
				if (this.currRow.awbNumber) {
					...
					center: true
						...
						this.$axios.get('/afbase/afoporder/view/' + this.currRow.orderId)
						.then(function(response) {

              var dataRow = response.data.data;
							var orderUUID = dataRow.orderUuid;
		            	 	var letterIds = '';
		            	 	var flag=1;
		            	 	for(var i = 0; i < dataRow.shipperLetters.length; i++) {
		            	 		var letter=dataRow.shipperLetters[i];
			            	 	if(letterIds == '') {
					              letterIds = letter.slId;
					            } else {
					              letterIds = letterIds + ',' + letter.slId;
					            }
					            if (!letter.hawbNumber
					            	||!letter.expectFlight
					            	||!letter.expectDeparture
					            	||!letter.departureStation
					            	||!letter.arrivalStation
					            	||!letter.loadingDate
					            	||!letter.planPieces
					            	||!letter.planWeight
					            	||!letter.planVolume
					            	||!letter.customsStatusCode
					            	||!letter.paymentMethod
					            	||!letter.afOrderShipperConsignee2.scPrintRemark
					            	||!letter.afOrderShipperConsignee1.scPrintRemark
					            	||!letter.goodsNameEn) {
					            	flag=0
					            	break;
					            }
		                	...
		                	this.doEsdDecleare(orderUUID,letterIds);
						}.bind(this));
			...
			doEsdDecleare(orderUUID,letterIds){
				var type='mwb';
				if (letterIds) {
					type='all'
				}
				this.$axios.post("/afbase/send/doEsdDecleare/"+type+"/"+orderUUID+'/'+letterIds).then(function (response) {
		            if (response.data.code == 0) {
		                if(response.data.data.status=='success'){
			...
			doSendEAWB_WH() { //发送舱单20200827
				if (this.currRow.awbNumber) {
					this.$confirm('此操作将发送交货入库, 是否继续?', '注意', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning',
					center: true
              ...
              var dataRow = response.data.data;
		                	...
		                	this.doEAWB_WH(orderUUID,letterIds);
						}.bind(this));
			...
			doEAWB_WH(orderUUID,letterIds){
				var type='mwb';
				if (letterIds) {
					type='all'
				}
				this.$axios.post("/afbase/send/doEAWB_WH/"+type+"/"+orderUUID+'/'+letterIds).then(function (response) {
		            if (response.data.code == 0) {
		                if(response.data.data.status=='success'){
		                  this.openSuccess('发送成功');
			            }else{
			...
			doSendOneDecleare() { //发送舱单20200827
				if (this.currRow.awbNumber) {
					this.$confirm('此操作将发送一键申报, 是否继续?', '注意', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning',
					center: true
              ...
              var dataRow = response.data.data;
		                	...
		                	this.doOneDecleare_ForSend(orderUUID,letterIds);
						}.bind(this));
			...
			doOneDecleare_ForSend(orderUUID,letterIds){
				var type='mwb';
				if (letterIds) {
					type='all'
				}
				this.$axios.post("/afbase/send/doOneDecleare_ForSend/"+type+"/"+orderUUID+'/'+letterIds).then(function (response) {
		            if (response.data.code == 0) {
		                if(response.data.data.status=='success'){
		                  this.openSuccess('发送成功');
			            }else{
			...
			doMft2201_Decleare(orderUUID,letterIds){
				...
				this.$axios.post("/afbase/send/doMft2201_Decleare/"+type+"/"+orderUUID+'/'+letterIds).then(function (response) {
		            if (response.data.code == 0) {
		                if(response.data.data.status=='success'){
		                  this.openSuccess('发送成功');
			            }else{
				...
				if (this.currRow.awbNumber) {
					this.$axios.get('/afbase/afoporder/getShippingData/AE_CD_POST_MAWB').then(function(response) {
						if (response.data.code == 0 && response.data.data) { //发送舱单数据
							...
							this.$axios.get("/afbase/afoporder/shippingSendCheckStatus/" + orderUUID).then(function(response) {
								function extracted() {
			...
			sendWaybillAndLog(orderUUID, data) {
				...
				this.$axios.post("/afbase/afoporder/insertAfLog", params).then(function(response) {
					if (response.data.code == 0) {
						this.openSuccess("发送成功!");
					} else {
				...
				this.$axios.get("/afbase/afoporder/shippingBillDataCheck/AE_CD_POST_MAWB/all/" + orderUUID).then(function(response) {
					function extracted() {
						this.$axios.get("/afbase/afoporder/shippingBillData/" + orderUUID).then(function(response) {
							if (response.data.code == 0) {
								...
								if (data['url']) {
									// 调用发送舱单后台接口 TODO
									this.sendWaybillAndLog(orderUUID, data);
								} else {
									this.openError1("发送舱单失败!");
								}
							} else {
								this.openError1(response.data.messageInfo || "发送舱单失败!");
							}
						}.bind(this));
					}
					...
					if (response.data.code == 0) {
						if (response.data.data == null) {
							...
							this.$confirm(response.data.data + '制作舱单可能会出现数据错误，请确认是否继续？', '提示', {
								confirmButtonText: '确定',
								cancelButtonText: '取消',
								dangerouslyUseHTMLString: true,
								customClass: 'message-logout',
			...
			sendWaybill(orderUUID) {
				this.$axios.get("/afbase/afoporder/shippingBillDataCheck/AE_DZ_POST_MAWB/all/" + orderUUID).then(function(response) {
					function extracted() {
						this.$axios.get("/afbase/afoporder/waybillData/" + orderUUID).then(function(response) {
							if (response.data.code == 0) {
								let data = response.data.data;
								if (data['url']) {
									this.openFormWind(data['url'], data, "_blank");
			...
			init() {
				...
				if (buttonInfo.indexOf('af-oporder-add') > -1) {
					this.addButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-excel') > -1) {
					this.excelButtonFlag = true;
				}

				if (buttonInfo.indexOf('af-oporder-view') > -1) {
				...
				if (buttonInfo.indexOf('af-oporder-uninstall') > -1) {
					this.uninstallButtonFlag = true;
				}
				// if (buttonInfo.indexOf('af-oporder-stop')>-1) {
				// 	this.stopButtonFlag=true;
				// }
				if (buttonInfo.indexOf('af-oporder-letter') > -1) {
				...
				if (buttonInfo.indexOf('af-oporder-forcestop') > -1) {
					this.forcestopButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-match') > -1) {
					this.matchButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-printletter') > -1) {
					this.printletterButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-awbsubmit') > -1) {
					this.awbsubmitButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-dgdMake') > -1) {
					this.dgdMakeButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-tagMake') > -1) {
					this.tagMakeButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-amountBillPrint') > -1) {
					this.amountBillPrintButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-amountBillSend') > -1) {
					this.amountBillSendButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-twoCode') > -1) {
					this.twoCodeMakeButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-checkTwoCode') > -1) {
					this.checkTwoCodeMakeButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-shareTwoCode') > -1) {
					this.shareTwoCodeButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-shippingMake') > -1) {
					this.shippingMakeButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-shippingSend') > -1) {
					this.shippingSendButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-waybillMake') > -1) {
				...
				if (buttonInfo.indexOf('af-oporder-print') > -1) {
					this.printButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-delivery') > -1) {
					this.deliveryButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-documentMake') > -1) {
					this.documentMakeButtonFlag = true;
				}
				if (buttonInfo.indexOf('af-oporder-special') > -1) {
				...
				if (buttonInfo.indexOf('af-oporder-services-income') > -1) {
					this.incomeServicePermissionButton = true
				}

				if (buttonInfo.indexOf('af-oporder-share-ae') > -1) {
			...
			deliveryNotice(row, flag) {
				this.$axios.get('/afbase/afoporder/view/' + row.orderId)
					.then((response) => {
						if (response.data.code == 0) {
							...
							this.$axios.get("/afbase/afoporder/checkOrderDeliveryNotice/" + row['orderUuid'] + "/" + flag).then((resp) => {
								if (resp.data.code == 0) {
									this.frow = row;
									this.frow.flag = flag
		...
		created() {
			...
			if (this.$route.query.flag) {
				...
				if (this.$route.query.name == 'dgdMake') {
					if (buttonInfo == null || buttonInfo.indexOf('af-oporder-dgdMake') == -1) {
					...
					if (buttonInfo == null || buttonInfo.indexOf('af-oporder-waybillMake') == -1) {
						this.$router.push("/noPermission")
						return
					}
