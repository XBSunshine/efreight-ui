<template>
	<el-dialog title="" width="975px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
		<div class="orderInquireSavePage">
			<el-form :model="ruleForm" :rules="rules" :inline="true" ref="ruleForm" label-width="100px" class="demo-ruleForm" style="margin-left: 10px;">
				<div style="margin-top: -15px;margin-bottom: 20px;">
					<span style="font-size: large;margin-bottom: 20px;">空运出口询价单 - 新增</span>
					...
					<el-col class="elementWidth">
						<el-form-item>
							<el-input class="widthWithThree" style="width:295px;margin-right: 15px;">
								...
								<el-select slot="suffix" remote :filter-method="searchAirport"  filterable :loading="loading" v-model="ruleForm.departureStation" placeholder="请输入关键字" style="width:198px;margin-right: -5px;">
					...
					<el-col class="elementWidth">
						<el-form-item prop="arrivalStation">
							<el-input class="widthWithThreeWithXing" style="width:295px;">
								...
								<el-select slot="suffix" remote :filter-method="searchAirport"  filterable v-model="ruleForm.arrivalStation" clearable placeholder="请输入关键字" style="width:112px;margin-right: -5px;">
                  <el-option v-for="item in airportList" :key="item.apCode" :value="item.apCode">
                    <el-row>
                      <el-col :span="2" style="width: unset;margin-right: 10px">{{item.apCode}}</el-col>
                      <el-col :span="10" style="overflow-x: hidden; width: unset; margin-right: 10px">{{item.apNameEn}}</el-col>
				...
				<el-row>
					<el-form-item>
						<el-button :loading="loading" type="primary" size="small" @click="submitForm('ruleForm')">保存并询盘</el-button>
					</el-form-item>
				</el-row>
			</el-form>
...
<script>
	...
	export default {
      ...
      prmInquiryAgentDef(departureStation){
          this.$axios.get('/afbase/orderInquiry/getInquryAgentDepList/'+departureStation).then(function(response){
            if(response.data.data){
              this.ruleForm.inquiryAgentIds = response.data.data.inquiryAgentIds;
              this.ruleForm.inquiryAgentNames = response.data.data.inquiryAgentNames;
            }
          }.bind(this));
      }
		},
        ...
        this.initAirportList.forEach((item)=>{
          this.airportList.push(item);
			...
			if (this.frow.exist) {
				...
				this.ruleForm.planWeight = this.frow.planWeight
				this.ruleForm.planVolume = this.frow.planVolume
				this.ruleForm.expectDeparture = this.frow.expectDeparture
				this.carrierCode = this.frow.carrierCode.split(',')
				this.ruleForm.planPieces = this.frow.planPieces
				this.setPlanChargeWeight()
        //默认询盘代理
        this.prmInquiryAgentDef(this.ruleForm.departureStation)
			}
