...
<script>
	import columns from './inquiry_order_column.json'
	...
	export default {
        ...
        localStorage.removeItem(userId+'_inquiry_list_column_width');//清除掉页面拖动字段宽度的缓存
				this.findByPage();
          ...
          localStorage.removeItem(userId+'_inquiry_list_column_width');//清除掉页面拖动字段宽度的缓存
				/*	this.multipleSelection = this.sortBykey(this.multipleSelection, 'index');
					localStorage.setItem(userId + '_af_inquiry_order_column', JSON.stringify(this.multipleSelection));
					this.handleClose();*/
				} else {
		...
		created() {
			//let userId = window.localStorage.getItem('userId');
			let inquiryOrder = JSON.stringify(columns.inquiryOrder);
			this.oldTableColumns = inquiryOrder;
      this.tableColumns = JSON.parse(inquiryOrder);
      this.tableColumns = this.sortBykey(this.tableColumns, 'index');
		...
		mounted() {
			let userId = window.localStorage.getItem('userId');
			let inquiry_list_column_width = window.localStorage.getItem(userId + '_inquiry_list_column_width');
      //从数据库查询设置信息
      let pageName = '订单操作管理/在线询价/询价管理';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.inquiry_list_column = response.data.data;

              //如果inquiry_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(inquiry_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(inquiry_list_column_width);
              ...
              if(this.inquiry_list_column && this.inquiry_list_column.length>0){
                  let loaclJsonArray = this.inquiry_list_column;
			...
			/*if (inquiry_order_column) {
				let loaclJsonArray = JSON.parse(inquiry_order_column);
				if (loaclJsonArray && loaclJsonArray.length > 0) {
					let liftNo = [];
					let lift = [];
					for (var i = 0; i < this.tableColumns.length; i++) {
