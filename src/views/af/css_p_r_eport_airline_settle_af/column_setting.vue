...
<script>
	import columns from './css_p_r_eport_airline_settle_af_column.json'
	...
	export default {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_css_p_r_eport_settle_af_list_column_width_airline'); //清除掉页面拖动字段宽度的缓存
				this.findByPage();
		...
		mounted() {
			this.setLabel(this.tableColumns);
			let userId = window.localStorage.getItem('userId');
			let af_list_column_width = window.localStorage.getItem(userId + '_css_p_r_eport_settle_af_list_column_width_airline');
			//从数据库查询设置信息
			let pageName = '报表统计分析/航线结算报表';
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
