							...
							<el-col class="elementWidth">
								<el-form-item label-width="10px">
									<el-input style="width:190px;">
										...
										<el-select slot="suffix" v-model="query.businessScope" placeholder="请选择" style="width:121px;margin-right: -5px;" @change="businessScopeChange" disabled>
              ...
              <el-col class="elementWidth" v-if="query.businessScope=='AE'">
                <el-form-item label-width="10px">
                  <el-input v-model="query.awbFromName" clearable auto-complete="off" style="width:189px;" maxlength="50" show-word-limit>
							...
							<el-col class="elementWidth">
								<el-form-item v-if="query.businessScope!='LC'&&query.businessScope!='IO'" label-width="12px">
...
<script>
	import setVisibleVue from './css_p_r_eport_airline_settle_af/column_setting'
	...
	import columns from './css_p_r_eport_airline_settle_af/css_p_r_eport_airline_settle_af_column.json'
		...
		/*			if (column.prop == 'mainRoutingGrossProfitWeight') {
			...
			cellWidth(newWidth, oldWidth, column, event) {
				...
				if (arrayC && arrayC.length > 0) {
					...
					localStorage.setItem(userId + '_css_p_r_eport_settle_af_list_column_width_airline', JSON.stringify(arrayC))
				}
			},
			setting() {
        ...
        this.query.orderPermission = orderPermission;
				...
				this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
					.then(function(response) {
						...
						this.$axios.post('/afbase/cssPReportSettleAf/airlineSettle', this.query).then((response) => {
							this.loading = false
        ...
        this.query.reportType = 'airline';
				this.$axios.post3('/afbase/cssPReportSettleAf/exportExcel', this.query)
					.then((response) => {
						...
						downloadElement.download = '航线结算报表' + '.xls'; // 下载后文件名
