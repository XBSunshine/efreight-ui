<template>
	<el-dialog title="仓库货站 - 编辑" width="1200px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
		<el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="90px" class="afWareHouseEdit">
			<el-container>
...
<script>
	export default {
		...
		created: function() {
			...
			this.customsCode2 = this.frow.customsCode;
			this.ruleForm.warehouseContactRemark = this.frow.warehouseContactRemark;
			this.ruleForm.delivererContactRemark = this.frow.delivererContactRemark;
			this.ruleForm.delivererContactName = this.frow.delivererContactName;
			this.ruleForm.delivererContactPhoneNumber = this.frow.delivererContactPhoneNumber;
			this.ruleForm.shipperTemplate = this.frow.shipperTemplate;
		...
		methods: {
			...
			submitForm(formName) {
				if (this.ruleForm.customsCode === this.customsCode2) {
					this.ruleForm.isChange = 0;
				} else {
					this.ruleForm.isChange = 1;
				}
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post2('/afbase/warehouse/doUpdate', this.ruleForm)
...
<style type="text/css">
	...
	.afWareHouseEdit .widthFive1 .el-input-group__prepend {
		padding: 0 18px;
	}

	.afWareHouseEdit .widthSix1 .el-input-group__prepend {
		padding: 0 13px;
	}
	...
	.afWareHouseEdit .delivererContactRemark textarea {
		height: 30px;
