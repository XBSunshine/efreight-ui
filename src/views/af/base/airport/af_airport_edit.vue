<template>
	<el-dialog title="机场 - 编辑" width="750px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
		<el-form :disabled="loading" :model="ruleForm" :rules="rules" ref="ruleForm" style="margin-left: 10px;" class="afAirportEdit-ruleForm">
			<el-row>
	...
	export default {
		...
		methods: {
			getAirport() {
              ...
              this.ruleForm.nationCodeThree = response.data.data.nationCodeThree;
              this.ruleForm.nationCodeNumber = response.data.data.nationCodeNumber;
              this.ruleForm.nationCodeCoo = response.data.data.nationCodeCoo;
						}
					}).then(() => {});
			},
			handleClose() {
				...
				this.$axios.get2("/afbase/airport/getAirportName", {
					...
					.then((response) => {
						if(response.data.code == 0 && response.data.data) {
							...
							this.ruleForm['apStatus'] = data['apStatus'] + "";
						} else {
			...
			loadRouting1() {
				/*let val = this.ruleForm.routingGroupName;
				if(!val) {
					return
				}*/
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						this.$axios.post2("/afbase/airport/doUpdate", params)
							.then(function(response) {
...
<style type="text/css">
  ...
  .afAirportEdit-ruleForm .widthFour1 .el-input-group__prepend {
    padding: 0 25px;
  }
  ...
  .afAirportEdit-ruleForm .widthFive .el-input-group__prepend {
    padding: 0 4px;
  }

  .afAirportEdit-ruleForm .widthFive1 .el-input-group__prepend {
    padding: 0 18px;
  }
