...
<script>
  import columns from './af_shipperConsignee_column.json'
	...
	export default {
        ...
        multipleSelection: [],
        oldTableColumns:'',
        af_shipperConsignee_list_column: []
			}
		},
   inject: ['queryList'],
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_af_shipperConsignee_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.queryList();
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  af_shipperConsignee_list_column_width = window.localStorage.getItem(userId+'_af_shipperConsignee_list_column_width');
      //从数据库查询设置信息
      let pageName = '客商资料管理/收发货人';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.af_shipperConsignee_list_column = response.data.data;

              //如果af_shipperConsignee_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(af_shipperConsignee_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(af_shipperConsignee_list_column_width);
              ...
              if(this.af_shipperConsignee_list_column && this.af_shipperConsignee_list_column.length>0){
                  let loaclJsonArray = this.af_shipperConsignee_list_column;
