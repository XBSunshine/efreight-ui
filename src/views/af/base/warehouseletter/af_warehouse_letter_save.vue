<template>
	<el-dialog title="货站托书 - 新增" width="1100px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose">
		<el-form :disabled="loading" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="90px" class="afWarehouseLetterSave">
			<el-row>
				...
				<el-col class="elementWidth">
					<el-form-item label="托书模板 (PDF)" prop="shipperTemplateFile" label-width="138px" class="shipperTemplateFileColor">
						...
						<el-upload v-if="ruleForm.shipperTemplateFile==null||ruleForm.shipperTemplateFile==''" :action="actionPath" :accept="accept" :limit=1 :data="uptoken" :before-upload="beforeAvatarUpload" :on-remove="handleRemoveChange" :on-success="handleSuccessChange" ref="upload" :auto-upload="true" :show-file-list="true">
							<el-button size="mini" type="primary" style="margin-left: 17px;">上传托书模板</el-button>
						</el-upload>
...
<script>
	export default {
		...
		data() {
		  const checkAirlineShipperLetterData = (rule, value, callback) =>{
      };
			return {
          ...
          return;
				...
				this.$refs[formName].validate((valid) => {
					if(valid) {
						...
						this.$axios.post2('/afbase/warehouseLetter/doSave', params)
							.then(function(response) {
