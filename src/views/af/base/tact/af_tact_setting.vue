...
<script>
  import columns from './af_tact_column.json'
   ...
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_af_tact_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.queryList();
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  af_tact_list_column_width = window.localStorage.getItem(userId+'_af_tact_list_column_width');
      //从数据库查询设置信息
      let pageName = '基础数据维护/空运信息/TACT';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.af_tact_list_column = response.data.data;

              //如果af_tact_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(af_tact_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(af_tact_list_column_width);
              ...
              if(this.af_tact_list_column && this.af_tact_list_column.length>0){
                  let loaclJsonArray = this.af_tact_list_column;
