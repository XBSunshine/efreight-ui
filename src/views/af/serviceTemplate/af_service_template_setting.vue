...
<script>
  import columns from './af_service_template_column.json'
	...
	export default {
        ...
        multipleSelection: [],
        oldTableColumns:'',
        af_service_template_list_column: []
			}
		},
   inject: ['findByPage'],
   provide() {
    ...
    checkNumber(row,p){
      ...
      handleClose() {
        ...
        localStorage.removeItem(userId+'_af_service_template_list_column_width');//清除掉页面拖动字段宽度的缓存
      	this.findByPage();
    ...
    mounted(){
      let  userId = window.localStorage.getItem('userId');
      let  af_service_template_list_column_width = window.localStorage.getItem(userId+'_af_service_template_list_column_width');
      //从数据库查询设置信息
      let pageName = '基础数据维护/公共信息/服务模板';
      this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
          .then(function(response) {
              this.af_service_template_list_column = response.data.data;

              //如果af_service_template_list_column_width不为空，则说明在页面拖动过字段的宽度
              if(af_service_template_list_column_width){
                  let loaclWidthJsonArray = JSON.parse(af_service_template_list_column_width);
              ...
              if(this.af_service_template_list_column && this.af_service_template_list_column.length>0){
                  let loaclJsonArray = this.af_service_template_list_column;
