<template>
	<el-dialog title="服务模板 - 修改" width="1290px" :visible.sync="visible" :close-on-press-escape="false" :close-on-click-modal="false" :before-close="handleClose" append-to-body>
			...
			<el-table :data="data1" :header-cell-style="{background:'oldlace'}" :row-class-name="tableRowClassName" >
				...
				<el-table-column label="收款客户" width="290" v-if="ruleForm.templateType==1">
					<template slot-scope="scope">
						<el-select :ref="'receiveSelectcustomerId'+scope.row.index" @keyup.native.enter="nextFocus($event,scope.row,'customerId','receive')" :default-first-option=true v-model="scope.row.customerId" filterable clearable placeholder="请选择客户" style="width:280px;">
							<el-option v-for="item in customerNameOptions" :key="item.coopIdV" :label="item.value" :value="parseInt(item.coopIdV)">
							</el-option>
						</el-select>
					</template>
				</el-table-column>
				...
				<el-table-column align="center" label="币种" width="90px" >
					<template slot-scope="scope">
						...
						<el-select :ref="'receiveSelectserviceCurrency'+scope.row.index" @keyup.native.enter="nextFocus($event,scope.row,'serviceCurrency','receive')" :default-first-option=true v-model="scope.row.serviceCurrency"  placeholder="选择" >
						...
						<div v-if="scope.row.serviceCurrency!='CNY'">
						<el-select class="currencyRedColor" :ref="'receiveSelectserviceCurrency'+scope.row.index" @keyup.native.enter="nextFocus($event,scope.row,'serviceCurrency','receive')" :default-first-option=true v-model="scope.row.serviceCurrency"  placeholder="选择" >
							<el-option v-for="item in currencyCodeOptions" :key="item.currency_code" :label="item.currency_code" :value="item.currency_code" >
							</el-option>
						</el-select>
...
<script>
	export default {
		...
		created: function() {
			this.ruleForm.businessScope=this.frow.row.businessScope;
			this.ruleForm.templateType=this.frow.row.templateType;

			// this.customerNameOptions1 = this.frow.customerNameOptions1;
			// this.customerNameOptions2 = this.frow.customerNameOptions2;
			// this.customerNameOptions = this.frow.customerNameOptions1;
			// this.setvalue1();
			this.$nextTick(() => {
				this.initInputDOM();
			});
			...
			this.$axios.get2('/afbase/service/queryList', {
				businessScope: this.frow.row.businessScope
        ...
        this.serviceOptions = this.serviceOptions.filter(port => port.isSys != true)
			...
			this.$axios.get2('/afbase/awb/queryList', {
				...
				awbFromName: this.frow.row.businessScope
			}).then(function(response) {
			...
			this.$axios.get('/afbase/awb/selectCategory/收费标准/'+this.frow.row.businessScope).then(function(response) {
			...
			this.$axios.post2('/afbase/servicetemplate/getView', this.frow.row).then(function(response) {
				this.ruleForm=response.data.data;
				this.ruleForm.deleteTemplate=[];
				this.data1 = response.data.data.addTemplate;
			}.bind(this));
		},
		...
		methods: {
			...
			clearData(){
				for (var i = 0; i < this.data1.length; i++) {
					if (this.data1[i].templateId) {
						this.ruleForm.deleteTemplate.push(this.data1[i]);
					}
				}
          ...
          serviceAmountCarry:'四舍五入'
			...
			doDeleteIncome(index, rrow) {
				if (rrow.templateId) {
					this.ruleForm.deleteTemplate.push(rrow);
				}
            ...
            if(this.data1[i].serviceAmountDigits === '') {
            	this.openError("请选择小数位");
            	return;
            }
            if(this.data1[i].serviceAmountCarry === '') {
            	this.openError("请选择进位方式");
            	return;
            }
					}
			...
			doSave(){
				this.$axios.post2('/afbase/servicetemplate/doEdit', this.ruleForm)
				.then(function(response) {
