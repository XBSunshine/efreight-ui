	...
	export default {
		...
		methods: {
			...
			handleClose() {
				...
				localStorage.removeItem(userId + '_invoice_writeoff_list_column_width'); //清除掉页面拖动字段宽度的缓存
				this.queryList();
		...
		mounted() {
			this.setLabel(this.tableColumns);
			let userId = window.localStorage.getItem('userId');
			let af_list_column_width = window.localStorage.getItem(userId + '_invoice_writeoff_list_column_width');
			//从数据库查询设置信息
			let pageName = '财务结算管理/收入对账/发票核销单';
			this.$axios.get2("/hrs/user/getUserPageSet?pageName=" + pageName)
				.then(function(response) {
